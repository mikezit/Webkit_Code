...
void CSSParser::setupParser(const char* prefix, const String& string, const char* suffix)
{
    ...
    fastFree(m_data);
    m_data = static_cast<UChar*>(fastMalloc(length * sizeof(UChar))); // copy prefix to m_data
    for (unsigned i = 0; i < strlen(prefix); i++)
        m_data[i] = prefix[i];
    // copy string to m_data
    memcpy(m_data + strlen(prefix), string.characters(), string.length() * sizeof(UChar));

    unsigned start = strlen(prefix) + string.length(); // copy suffix to m_data
    ...
    yy_hold_char = *yy_c_buf_p;	// 设置当前正在扫描的字符为从第一个开始
}
...
void CSSParser::parseSheet(CSSStyleSheet* sheet, const String& string)
{
    ...
    setupParser("", string, ""); // 设置要解析的css的字符串，设置解析参数
    cssyyparse(this);
    m_rule = 0;
}
