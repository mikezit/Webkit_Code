...
void SegmentedString::setExcludeLineNumbers()
{
 ...
 void SegmentedString::j_print() const
 {
    const UChar* mystr = toString().impl()->characters();
    if(toString().impl() == NULL)
	 printf("NULL String");
    else{
	 int len = toString().length();
	 printf(" segmentstring:\n ");
	 for(int i=0;i<len;i++){
	      char c = (char)(mystr[i]);
	      printf("%c",c);
	      if(c == '\0')
...
void SegmentedString::j_printSubstrings() const
{
     Deque<SegmentedSubstring>::const_iterator it = m_substrings.begin();
     Deque<SegmentedSubstring>::const_iterator e = m_substrings.end();
     printf("sub segmentstring:\n ");
