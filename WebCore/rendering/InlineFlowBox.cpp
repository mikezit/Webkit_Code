...
int InlineFlowBox::placeBoxesHorizontally(int xPos, bool& needsWordSpacing)
{
    ...
    // 循环设置每个孩子的位置
    for (InlineBox* curr = firstChild(); curr; curr = curr->nextOnLine()) {
        if (curr->renderer()->isText()) { // 如果这个孩子是文字
            InlineTextBox* text = static_cast<InlineTextBox*>(curr);
            RenderText* rt = toRenderText(text->renderer());
            if (rt->textLength()) { // 词间距距离加入
    ...
    setWidth(xPos - startX);	// 设置该flowbox的宽
                                             ...
                                             int& maxAscent, int& maxDescent, bool strictMode)
{
    if (isRootInlineBox()) {
        ...
        int height = lineHeight(true); // 一行字的高度
        int baseline = baselinePosition(true); // 得到字体baseline的位置
        if (hasTextChildren() || strictMode) {
            int ascent = baseline; // 字体ascent高度
            int descent = height - ascent; // 字体descent高度
            if (maxAscent < ascent)  // 设置最大ascent
                maxAscent = ascent;
            if (maxDescent < descent) // 设置最大descent
                maxDescent = descent;
        }
    }
    ...
    for (InlineBox* curr = firstChild(); curr; curr = curr->nextOnLine()) {
        ...
        if (usedFonts) {	// 如果使用了其他的字体
            ...
            lineHeight = curr->lineHeight(false); // 得到line高度(replace对象是返回高度加margin)
            baseline = curr->baselinePosition(false);
        }

        curr->setY(verticalPositionForBox(curr, m_firstLine)); // 设置当前inline box的Y坐标
        if (curr->y() == PositionTop) {
...
void InlineFlowBox::placeBoxesVertically(int yPos, int maxHeight, int maxAscent, bool strictMode, int& selectionTop, int& selectionBottom)
{
    ...
    for (InlineBox* curr = firstChild(); curr; curr = curr->nextOnLine()) { // 放置孩子的位置
        if (curr->renderer()->isPositioned())
            continue; // Positioned placeholders don't affect calculations.
        ...
        bool isInlineFlow = curr->isInlineFlowBox();
        if (isInlineFlow)
            static_cast<InlineFlowBox*>(curr)->placeBoxesVertically(yPos, maxHeight, maxAscent, strictMode, selectionTop, selectionBottom); // 递归

        bool childAffectsTopBottomPos = true; // 孩子是否影响了top和bottom的位置
        if (curr->y() == PositionTop)
            curr->setY(yPos);
        else if (curr->y() == PositionBottom)
		 ...
		 childAffectsTopBottomPos = false;// in if
            int posAdjust = maxAscent - curr->baselinePosition(false); // not in if
...
void InlineFlowBox::paint(RenderObject::PaintInfo& paintInfo, int tx, int ty)
{
    ...
    overflowRect.move(tx, ty);	// 把overflowRect移动到屏幕坐标
...
void InlineFlowBox::paintBoxDecorations(RenderObject::PaintInfo& paintInfo, int tx, int ty)
{
    ...
    int x = m_x; // m_x ,m_y ,是在InLineBox里面定义
    int y = m_y;
    int w = width();
    int h = height();
