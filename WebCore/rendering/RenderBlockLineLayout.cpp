...
// 针对obj对象创建一个inlineBox
static inline InlineBox* createInlineBoxForRenderer(RenderObject* obj, bool isRootLineBox, bool isOnlyRun = false)
{
    if (isRootLineBox)	      	// rootLineBox
    ...
    return toRenderInline(obj)->createAndAppendInlineFlowBox(); // InLine
}
...
InlineFlowBox* RenderBlock::createLineBoxes(RenderObject* obj, bool firstLine)
{
    ...
    do {
        ...
        parentBox = obj->isRenderInline() ? toRenderInline(obj)->lastLineBox() : toRenderBlock(obj)->lastLineBox(); // 之前构造有的话，会直接取出来,没有构造的话,返回null，则在下面会进行构造
        ...
        // inline.
        if (childBox) // 如果有之前构造的孩子，则将其加入到parentBox之中
            parentBox->addToLine(childBox);

        if (!constructedNewBox || obj == this) // 如果没有创建新的box或则obj已经是最外层的block
            break;
...
RootInlineBox* RenderBlock::constructLine(unsigned runCount, BidiRun* firstRun, BidiRun* lastRun, bool firstLine, bool lastLine, RenderObject* endObject)
{
    ...
    for (BidiRun* r = firstRun; r; r = r->next()) {
        ...
        if (box->isInlineTextBox()) {
            ...
            text->setStart(r->m_start);		  // 设置字符窜在text的开始位置
            text->setLen(r->m_stop - r->m_start); // 设置字符窜的长度
...
void RenderBlock::computeHorizontalPositionsForLine(RootInlineBox* lineBox, bool firstLine, BidiRun* firstRun, BidiRun* trailingSpaceRun, bool reachedEnd)
{
    ...
    int availableWidth = lineWidth(height(), firstLine); // 可用宽度
    ...
    for (BidiRun* r = firstRun; r; r = r->next()) {
        ...
        if (r->m_object->isText()) {
            ...
            r->m_box->setWidth(rt->width(r->m_start, r->m_stop - r->m_start, totWidth, firstLine, &fallbackFonts, &glyphOverflow)); // 设在box的宽度为文字的长度
    ...
    // justifying text.
    int x = leftOffset(height(), firstLine); // 左边的起始位置
    switch (textAlign) {
        case LEFT:
        case WEBKIT_LEFT:
    ...
    lineBox->placeBoxesHorizontally(x, needsWordSpacing); // 设定linebox的位置(和linebox的所有孩子)
}

void RenderBlock::computeVerticalPositionsForLine(RootInlineBox* lineBox, BidiRun* firstRun)
{
    setHeight(lineBox->verticallyAlignBoxes(height())); // 设置Block的高度
    lineBox->setBlockHeight(height());	// 设置当前lineBox的高度
...
void RenderBlock::layoutInlineChildren(bool relayoutChildren, int& repaintTop, int& repaintBottom)
{
    ...
    // Walk all the lines and delete our ellipsis(省略) line boxes if they exist.
    if (hasTextOverflow)
    ...
    if (firstChild()) {
        ...
        RenderObject* o = bidiFirst(this, 0, false); // 得到第一个节点(不忽略inline)
        Vector<FloatWithRect> floats;
        bool hasInlineChild = false;
        while (o) {
            if (o->isReplaced() || o->isFloating() || o->isPositioned()) { // 对特殊节点进行处理
            ...
            } else if (o->isText() || (o->isRenderInline() && !endOfInline)) { // o 是text，或则o是inline且是第一次遍历的(没有经过上次处理)
                hasInlineChild = true; // 有孩子
            ...
            o = bidiNext(this, o, 0, false, &endOfInline); // 找到下一个inline节点
        }
        ...
        InlineIterator end = resolver.position(); // inline 对象遍历器
        ...
        while (!end.atEnd()) {	// 没有遍历完，继续遍历
            ...
            EClear clear = CNONE;
            end = findNextLineBreak(resolver, firstLine, isLineEmpty, previousLineBrokeCleanly, &clear); // 找到文本中下一行的换行位置
            if (resolver.position().atEnd()) { // 如果已经遍历到最后
                resolver.deleteRuns();
            ...
            if (!isLineEmpty) {	// 如果文字非空
                ...
                if (resolver.runCount()) { // runcount应该是resolver中包含的TextRun的个数
                    lineBox = constructLine(resolver.runCount(), resolver.firstRun(), resolver.lastRun(), firstLine, !end.obj, end.obj && !end.pos ? end.obj : 0); // 创建一行
                    if (lineBox) {
            ...
            lineMidpointState.reset();
            resolver.setPosition(end); // 更改resolver的位置
        }
...
// renderinline是否需要lineBox( 必须要有孩子,有水平的padding或margin)
...
{
    if (it.obj->isFloatingOrPositioned()) // 当前对象是floating ,positioned box
        return false;
    ...
    UChar current = it.current(); // 得到当前字符
...
{
    int availableWidth = lineWidth(height(), firstLine); // 得到可用的line的宽度
...
// firstLine:是否是第一行文字,isLineEmpty:判断当前行是否为空
                                              ...
                                              EClear* clear)
{
    ...
    int width = skipLeadingWhitespace(resolver, firstLine, isLineEmpty, previousLineBrokeCleanly); // 得到可用的宽度
    ...
    bool ignoringSpaces = false; // 是否忽略空白
    ...
    RenderObject *o = resolver.position().obj; // 当前遍历对象
    RenderObject *last = o;
    unsigned pos = resolver.position().pos; // 当前文字位置
    ...
    while (o) {
        ...
        bool collapseWhiteSpace = RenderStyle::collapseWhiteSpace(currWS); // 是否折叠空白
            
        if (o->isBR()) {	// o 是RenderBR (RenderText的子类)对象
            if (w + tmpW <= width) {
                lBreak.obj = o;
                lBreak.pos = 0;
        ...
        if (o->isFloatingOrPositioned()) { // o是floating或则positioned对象
        ...
        } else if (o->isRenderInline()) { // o是RenderInline对象
        ...
        } else if (o->isReplaced()) { // o 是Replaced对象
            ...
            if (o->isListMarker()) {
                ...
                tmpW += replacedBox->width() + replacedBox->marginLeft() + replacedBox->marginRight() + inlineWidth(o);
        } else if (o->isText()) { // o是text对象
            if (!pos)		  // 如果当前pos为0
                appliedStartWidth = false;

            RenderText* t = toRenderText(o);

            int strlen = t->textLength(); // 得到text长度
            int len = strlen - pos;	  // 得到剩余长度
            const UChar* str = t->characters(); // 得到text字符串

            const Font& f = t->style(firstLine)->font(); // 得到字体
            bool isFixedPitch = f.isFixedPitch();

            int lastSpace = pos; // 保留上个位置
            int wordSpacing = o->style()->wordSpacing(); // 字符间的间距?
            int lastSpaceWordSpacing = 0;

            // Non-zero only when kerning(n. 字距调整) is enabled, in which case we measure words with their trailing 
            ...
            if (t->isWordBreak()) { // t 是RenderWordBreak
                w += tmpW;
                tmpW = 0;
                lBreak.obj = o;
                lBreak.pos = 0;
            ...
            while (len) {
                bool previousCharacterIsSpace = currentCharacterIsSpace; // 上个字符是空格
                bool previousCharacterIsWS = currentCharacterIsWS; // 上个字符是WS
                UChar c = str[pos];
                currentCharacterIsSpace = c == ' ' || c == '\t' || (!preserveNewline && (c == '\n')); // 当前字符是否为空白字符

                if (!collapseWhiteSpace || !currentCharacterIsSpace) // 不进行空白重叠或则当前字符不是空白
                    isLineEmpty = false; // 设置当前line非空(有内容)
                
                // Check for soft hyphens.  Go ahead and ignore them.
                if (c == softHyphen) { // hyphen : 连字号
                    if (!ignoringSpaces) {
                        // Ignore soft hyphens
                ...
                bool betweenWords = c == '\n' || (currWS != PRE && !atStart && isBreakable(str, pos, strlen, nextBreakable, breakNBSP)); // 是否是在词和词的边缘
    
                if (betweenWords || midWordBreak) { // 是到达了一个词的末尾,计算现在字符的长度
                ...
                if (collapseWhiteSpace && currentCharacterIsSpace && !ignoringSpaces) // 如果要进行空白重叠,并且当前字符是空白,并且..
