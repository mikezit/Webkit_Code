...
struct BidiStatus;
struct GapRects;
/* 代表inline中多行中的一行(也可以是唯一的一行) ,该行由几个或则一个 InlineFlowBox 构成 */
...
//      
//   对于一个inline对象，(当一行显示不下所有的内容时)它可以有很多行，这些行存放在RenderBlock的
// RenderLineBoxList中,RenderLineBoxList中的每一元素是RootInlineBox对象，下图所示的是一行
// 在webkit中的结构表示，InLineBox是一行中的最小元素,它是一个Text元素，InLineFlowBox，对应于
// 一些inline对象，在它之中包含了InLineBox
//
//
//
// +----------------------------------------------------------------+
