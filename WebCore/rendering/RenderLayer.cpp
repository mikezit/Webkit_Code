...
void RenderLayer::updateLayerPosition()
{
    ...
    RenderBox* rendererBox = renderBox(); // 得到layer对应的renderbox
    
    int x = rendererBox ? rendererBox->x() : 0; // 得到box 的x,y坐标(相对于Container的坐标)
...
{
    if (!m_parent && renderer()->parent()) { // 如果父亲不存在，并且对应的render有父亲
...
RenderLayer::convertToLayerCoords(const RenderLayer* ancestorLayer, int& xPos, int& yPos) const
{
    ...
    EPosition position = renderer()->style()->position(); // 得到position的属性
    if (position == FixedPosition && (!ancestorLayer || ancestorLayer == renderer()->view()->layer())) { // position 是 fixed ,并且ancetorLayer是view层对应的Layer
    ...
    if (position == FixedPosition) {// position 是 fixed ,并且ancetorLayer不是view层对应的Layer
...
// 得到两个scrollbar中右下角空出的一个矩形
static IntRect cornerRect(const RenderLayer* layer, const IntRect& bounds)
{
    int horizontalThickness;	// 垂直滚动条的witdh
    int verticalThickness;	// 水平滚动条的height
...
void RenderLayer::positionOverflowControls(int tx, int ty)
{
    ...
    IntRect scrollCorner(scrollCornerRect(this, borderBox));// 两个scrollbar中右下角空出的一个矩形
    IntRect absBounds(borderBox.x() + tx, borderBox.y() + ty, borderBox.width(), borderBox.height()); // 把borderBox转换成屏幕上的绝对坐标
    if (m_vBar)			// 设置垂直滚动条的矩形
...
// 鼠标点是否在oveflow区域上的滚动条上测试
bool RenderLayer::hitTestOverflowControls(HitTestResult& result, const IntPoint& localPoint)
{
    if (!m_hBar && !m_vBar && (!renderer()->hasOverflowClip() || renderer()->style()->resize() == RESIZE_NONE)) // 没有滚动条，返回
        return false;
    ...
    if (m_vBar) {		// 垂直滚动条测试
        IntRect vBarRect(box->width() - box->borderRight() - m_vBar->width(), 
                         box->borderTop(),
                         m_vBar->width(),
                         box->height() - (box->borderTop() + box->borderBottom()) - (m_hBar ? m_hBar->height() : resizeControlSize)); // 垂直滚动条区域矩形
        if (vBarRect.contains(localPoint)) { // 该滚动条区域是否包含当前鼠标点
            result.setScrollbar(m_vBar.get()); // 如果包含，设置点击结果
            return true;
        }
    }
                        ...
                        PaintLayerFlags paintFlags)
{
    ...
    if (!renderer()->opacity())	// opacity:不透明
        return;
    ...
    PaintLayerFlags localPaintFlags = paintFlags & ~PaintLayerAppliedTransform; // 去掉移位
    bool haveTransparency = localPaintFlags & PaintLayerHaveTransparency; // 是否有透明度
    ...
    // Calculate the clip rects we should use.(damageRect:backgroundRect,clipRectToApply:foregroundRect)
    ...
    int x = layerBounds.x();	// 当前layer在rootLayer的x,y坐标
    int y = layerBounds.y();
    int tx = x - renderBoxX();	// tx,ty做何解释?
    int ty = y - renderBoxY();
    ...
    bool shouldPaint = intersectsDamageRect(layerBounds, damageRect, rootLayer) && m_hasVisibleContent && isSelfPaintingLayer(); // overflow 一般都不是SelfPaintingLayer
    if (shouldPaint && !selectionOnly && !damageRect.isEmpty()) {
        ...
        // Paint the background. (1 画自己的背景 对应render的孩子们不会画)
    ...
    if (shouldPaint && !clipRectToApply.isEmpty()) {
        ...
        renderer()->paint(paintInfo, tx, ty); // 画其所有孩子的背景
        if (!selectionOnly) {
...
// 只有rootlayer才会被调用hitTest,然后hitest再递归测试每一个子layer
bool RenderLayer::hitTest(const HitTestRequest& request, HitTestResult& result)
{
    ...
    //( 参数: rootLayer, containerLayer,request, result, hitTestRect hitTestPoint....)
    RenderLayer* insideLayer = hitTestLayer(this, 0, request, result, boundsRect, result.point(), false); // 因为rootlayer没有containerlayer，所以第二个参数为0
    if (!insideLayer) {
                                                ...
                                                const HitTestingTransformState* transformState, double* zOffset)
{
    ...
    IntRect layerBounds;	// 层包围矩形
    IntRect bgRect;		// 层背景矩形
    IntRect fgRect;		// 层前景矩形
    IntRect outlineRect;
    ...
    if (m_posZOrderList) {	// 对每个正z order层进行hittestlayer测试
    ...
    if (candidateLayer)		// 如果找到一层layer是被点击的，则返回
        return candidateLayer;

    if (bgRect.contains(hitTestPoint) && isSelfPaintingLayer()) { // HitTestSelf
        HitTestResult tempResult(result.point());
...
bool RenderLayer::hitTestContents(const HitTestRequest& request, HitTestResult& result, const IntRect& layerBounds, const IntPoint& hitTestPoint, HitTestFilter hitTestFilter) const
{
    if (!renderer()->hitTest(request, result, hitTestPoint,
        ...
        return false;		// 如果该层没有被点击，则返回false
    }
...
void RenderLayer::calculateClipRects(const RenderLayer* rootLayer, ClipRects& clipRects, bool useCached) const
{
    ...
    if (parentLayer) {
        if (useCached && parentLayer->clipRects())
            ...
            parentLayer->calculateClipRects(rootLayer, clipRects); // 又是一个递归计算
    }
    else
        clipRects.reset(ClipRects::infiniteRect()); // 递归基本条件
    ...
    if (renderer()->hasOverflowClip() || renderer()->hasClip()) {
        ...
        RenderView* view = renderer()->view();
        ASSERT(view);
        if (view && clipRects.fixed() && rootLayer->renderer() == view) { // 如果当前rootLayer是View,则计算出来的坐标还要减去滚动条的偏移
...
IntRect RenderLayer::backgroundClipRect(const RenderLayer* rootLayer, bool temporaryClipRects) const
{
    ...
    if (parent()) {
        ...
        parentClipRects(rootLayer, parentRects, temporaryClipRects); // 得到parentRects
...
// 计算出相对于rootLayer坐标的layerBounds，并且计算出background和foreground的clip rect
...
{
    if (rootLayer != this && parent()) { // 当前layer不是rootLayer
        backgroundRect = backgroundClipRect(rootLayer, temporaryClipRects);
        backgroundRect.intersect(paintDirtyRect);
    } else // 当前layer是rootLayer
        backgroundRect = paintDirtyRect;

    foregroundRect = backgroundRect; // 使foregroundRect等于backgroundRect
    outlineRect = backgroundRect;    // 使outlineRect等于backgroundRect
                                     // 如果当前layer对应的render没有clip,那么上面的计算就是结果
    int x = 0;
    int y = 0;
    convertToLayerCoords(rootLayer, x, y); // 把当前layer中的x,y坐标转换成rootLayer中的坐标
    layerBounds = IntRect(x, y, width(), height()); // rootLayer中当前layer的边界矩形
    
    // Update the clip rects that will be passed to child layers.(有clip的情况才会调用)
    if (renderer()->hasOverflowClip() || renderer()->hasClip()) {
        // This layer establishes a clip of some kind.
        if (renderer()->hasOverflowClip())
...
// 相对于ancestorLayer坐标空间的boundingBox
...
void RenderLayer::showRenderLayer() const
{
     ...
     char prefix[100];
     int i=0;
     for (const RenderLayer* tmpLayer = this; tmpLayer && tmpLayer != rootLayer; tmpLayer = tmpLayer->parent())
	  i++;
     memset(prefix,'\t',i);
     prefix[i]='\0';
...
void RenderLayer::showTreeForThis() const
{
     const RenderLayer* rootLayer = root();
     if(rootLayer == NULL)
	  return;
