...
// block的addChild主要处理使block的孩子全部是inline或则block，最后还是要调用RenderObject的addChild
void RenderBlock::addChild(RenderObject* newChild, RenderObject* beforeChild)
{
    // Make sure we don't append things after :after-generated content if we have it.
    if (!beforeChild) {	  // beforeChild为空,计算之
        RenderObject* lastRenderer = lastChild(); // 该box在containing box前一个box
    ...
    bool madeBoxesNonInline = false; //是否包装了inline child到匿名block中
    ...
    RenderBox::addChild(newChild, beforeChild); // 调用子类的addchild
...
void RenderBlock::layoutBlock(bool relayoutChildren) // 开始布局该block
{
    ASSERT(needsLayout());
    ...
    int oldWidth = width();	// 得到之前的width
    int oldColumnWidth = desiredColumnWidth();

    calcWidth();	       	// 计算现在的width
    calcColumnWidth();

    m_overflow.clear();		// overflow清空

    if (oldWidth != width() || oldColumnWidth != desiredColumnWidth())
        relayoutChildren = true; // 需要重新布局孩子

    clearFloats();		// 清除float block
    ...
    if (!isCell) {		// Talbe没有Margin
        initMaxMarginValues();	//初始化maxTopPos, maxTopNeg, maxBottomPos, and maxBottomNeg
    ...
    int repaintTop = 0;
    int repaintBottom = 0;
    int maxFloatBottom = 0;	// 本block中当前float放置的最低点
    if (childrenInline())
        layoutInlineChildren(relayoutChildren, repaintTop, repaintBottom); // inline孩子布局
    else
        layoutBlockChildren(relayoutChildren, maxFloatBottom); // block孩子布局
    ...
    layoutPositionedObjects(relayoutChildren || isRoot()); // 布局固定的对象
    ...
    setNeedsLayout(false);	// 该block已经布局了
}
...
bool RenderBlock::handlePositionedChild(RenderBox* child, const MarginInfo& marginInfo)
{
    if (child->isPositioned()) {
        child->containingBlock()->insertPositionedObject(child); // child 的containning block 有可能不是this
...
int RenderBlock::collapseMargins(RenderBox* child, MarginInfo& marginInfo)
{
    ...
    int posTop = child->maxTopMargin(true); // 正的绝对值
    int negTop = child->maxTopMargin(false); // 负的绝对值
    ...
    if (child->isSelfCollapsingBlock()) {
        ...
        if (child->style()->marginTopCollapse() == MSEPARATE) {
            ...
            setHeight(height() + max(marginInfo.posMargin(), posTop) - max(marginInfo.negMargin(), negTop)); // max是相邻的上个sibling和当前的child的margin进行margin重叠(marginInfo.posMargin()是上个child的bottomMargin,posTop是当前child的topMargin)
            ypos = height();
        }

        marginInfo.setPosMargin(child->maxBottomMargin(true)); // 把当前的margin设置为child的底边
...
{
    // FIXME: We need to eliminate the estimation of vertical position, because when it's wrong we sometimes trigger a pathological(病理学的)
    // relayout if there are intruding(侵入) floats.
    int yPosEstimate = height();
    if (!marginInfo.canCollapseWithTop()) { // 该child block无法和父block在top上进行margin重叠
    ...
    yPosEstimate += getClearDelta(child, yPosEstimate); // 针对float block
    return yPosEstimate;
}
...
{
    if (gPercentHeightDescendantsMap) { // 这个还不知道作用
    ...
    setHeight(top);		// 这个是暂时设置的高度(这个高度会随着每个孩子的添加而愈来愈大,同时,这个高度也用来估算后面的yPosEstimate,也就是孩子的y坐标值)

    // The margin struct caches all our current margin collapsing state.  The compact struct caches state when we encounter compacts,
    MarginInfo marginInfo(this, top, bottom); // top 和bottom用来判断是否可以和孩子进行margin重叠(是否有border或(和)padding)
    ...
    maxFloatBottom = 0;		// 初始化为0

    RenderBox* next = firstChildBox();

    while (next) {		// 对block的所有孩子进行布局
...
void RenderBlock::layoutBlockChild(RenderBox* child, MarginInfo& marginInfo, int& previousFloatBottom, int& maxFloatBottom)
{
    ...
    child->calcVerticalMargins(); // 计算marginTop 和 marginBottom(这里计算出来的是本来的值，没有经过什么重叠和其他的等式计算)

    // Do not allow a collapse if the margin top collapse style is set to SEPARATE(分开).
    if (child->style()->marginTopCollapse() == MSEPARATE) {
        marginInfo.setAtTopOfBlock(false);
    ...
    child->setLocation(child->x(), yPosEstimate); // 设置child的位置

    bool markDescendantsWithFloats = false; // child的孩子中有float
    if (yPosEstimate != oldRect.y() && !child->avoidsFloats() && child->isBlockFlow() && toRenderBlock(child)->containsFloats())
        ...
        // layout.
        int fb = max(previousFloatBottom, floatBottom()); // 当前float的最底边的高度
        if (fb > yPosEstimate)				  // 大于当前child的高度的话
            markDescendantsWithFloats = true;
    }
    ...
    if (child->isRenderBlock()) {
        ...
        previousFloatBottom = max(previousFloatBottom, oldRect.y() + toRenderBlock(child)->floatBottom()); // 重新计算当前float的最低边高度
    }

    bool childHadLayout = child->m_everHadLayout;
    bool childNeededLayout = child->needsLayout();
    if (childNeededLayout)
        child->layout();	// 开始布局child

    // Now determine the correct ypos based off examination of collapsing margin
    // values.
    int yBeforeClear = collapseMargins(child, marginInfo); // child和上个孩子或者cotainerbox的top进行margin重叠,yBeforeClear是重叠后的高
    ...
    determineHorizontalPosition(child); // 确定child的水平位置

    // Update our height now that the child has been placed in the correct position.
    setHeight(height() + child->height()); // 根据孩子的高度改变自己的高度(block 是垂直布局)
...
void RenderBlock::paint(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    bool pushedClip = pushContentsClip(paintInfo, tx, ty); // 对overflow的clipRect进行处理,会改变tx,ty的值进行处理,并且改变phase阶段
    paintObject(paintInfo, tx, ty);
    if (pushedClip)  // 如果进行了overflow处理，则复原pahse阶段
    ...
    if (hasOverflowClip() && style()->visibility() == VISIBLE && (phase == PaintPhaseBlockBackground || phase == PaintPhaseChildBlockBackground) && shouldPaintWithinRoot(paintInfo))
        layer()->paintOverflowControls(paintInfo.context, tx, ty, paintInfo.rect); // 画滚动条
}
...
void RenderBlock::paintContents(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (childrenInline())	// 孩子inline(画出内容)
        m_lineBoxes.paint(this, paintInfo, tx, ty);
    else			// 非inline，则画出每个孩子
        paintChildren(paintInfo, tx, ty);
}

void RenderBlock::paintChildren(PaintInfo& paintInfo, int tx, int ty)
{
    PaintPhase newPhase = (paintInfo.phase == PaintPhaseChildOutlines) ? PaintPhaseOutline : paintInfo.phase; // 如果是phase是PaintPhaseChildOutlines，转换成PaintPhaseOutline
    newPhase = (newPhase == PaintPhaseChildBlockBackgrounds) ? PaintPhaseChildBlockBackground : newPhase; // 如果phase是PaintPhaseChildBlockBackgrounds，转换成PaintPhaseChildBlockBackground
	...
	// 如果这个孩子没有SelfPaintingLayer( 没有layer), 并且不是floating
...
void RenderBlock::paintObject(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    // 1. paint background, borders etc (PaintPhaseBlockBackground:是指定画当前render的背景，PaintPhaseChildBlockBackground是画所有孩子背景时候，画自己的)
    if ((paintPhase == PaintPhaseBlockBackground || paintPhase == PaintPhaseChildBlockBackground) && style()->visibility() == VISIBLE) {
        if (hasBoxDecorations()) // 是否有背景
            paintBoxDecorations(paintInfo, tx, ty); // 画背景和边框
        if (hasColumns())
    ...
    if (paintPhase == PaintPhaseBlockBackground) // 如果只仅仅是画自己的背景，则不需要再继续
        return;	 

    // Adjust our painting position if we're inside a scrolled layer (e.g., an overflow:auto div).
    int scrolledX = tx;		// 滚动条的位置
    int scrolledY = ty;
    if (hasOverflowClip()) {	// 如果有overflow,则加上内容的偏移
        IntSize offset = layer()->scrolledContentOffset();
        scrolledX -= offset.width();
        scrolledY -= offset.height();
    }

    // 2. paint contents(第2步除开PaintPhaseSelfOutline阶段每次都要执行) 这一步的作用是，下面的每个阶段，都要先作用于该render object的孩子,除非该孩子自己有自己的layer
    if (paintPhase != PaintPhaseSelfOutline) {
        if (hasColumns())
            paintColumnContents(paintInfo, scrolledX, scrolledY);
        else
            paintContents(paintInfo, scrolledX, scrolledY); // paintContent里面会循环调用当前layer下的所有孩子的paint函数，目的是画背景和边框
    }
...
// point 点是否在over flow区域上的测试
bool RenderBlock::isPointInOverflowControl(HitTestResult& result, int _x, int _y, int _tx, int _ty)
{
    if (!scrollsOverflow())	// 非overflow区域
        return false;		// 则返回

    return layer()->hitTestOverflowControls(result, IntPoint(_x - _tx, _y - _ty)); // 是否在滚动条上的测试
}
...
{
    int tx = _tx + x();
    int ty = _ty + y();		// tx , ty 现在是相对于屏幕的坐标
    
    if (!isRenderView()) {  // 这一步测试当前block是否包含 _x,_y点,如果不包含,则返回false
        // Check if we need to do anything at all.
        IntRect overflowBox = visibleOverflowRect(); // 得到box的矩形(包括边框)
        overflowBox.move(tx, ty);		     // 移动的相对于屏幕的坐标
        if (!overflowBox.contains(_x, _y))	     // 是否block是否包含 _x,_y点
            return false;			     // 不包含,则返回false
    }

    if ((hitTestAction == HitTestBlockBackground || hitTestAction == HitTestChildBlockBackground) && isPointInOverflowControl(result, _x, _y, tx, ty)) { // 点击测试的是背景,或则是孩子的背景,并且该点在 overflow 的 scrollbar上
...
void RenderBlock::updateHitTestResult(HitTestResult& result, const IntPoint& point)
{
    ...
    if (n) {
        result.setInnerNode(n);	// 设置当前节点
        if (!result.innerNonSharedNode())
            result.setInnerNonSharedNode(n);
        result.setLocalPoint(point); // 设置点击点在当前区域的坐标
    }
}
