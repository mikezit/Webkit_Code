...
void RenderView::layout()
{
    ...
    bool relayoutChildren = !printing() && (!m_frameView || width() != viewWidth() || height() != viewHeight()); // width() != viewWidth() 说明用户改变了窗口大小(孩子需要进行重新布局)
    if (relayoutChildren) {
        setChildNeedsLayout(true, false); // 设置孩子需要进行重新布局
        for (RenderObject* child = firstChild(); child; child = child->nextSibling()) { // 检查是否有孩子使用百分比设置高度,如果有的话,设置该孩子需要进行重新布局
    ...
    LayoutState state;	// 创建新的布局状态
    // FIXME: May be better to push a clip and avoid issuing offscreen repaints.
    state.m_clipped = false;
    m_layoutState = &state;

    if (needsLayout())		// 如果需要进行布局
        RenderBlock::layout();	// 则开始进行布局
...
void RenderView::computeRectForRepaint(RenderBoxModelObject* repaintContainer, IntRect& rect, bool fixed)
{
    ...
    if (fixed && m_frameView)	// position = fixed 的box
