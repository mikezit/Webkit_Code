...
public:
    ...
    const RenderObjectChildList* children() const { return &m_children; } /* 返回孩子 */
    ...
    virtual void addChild(RenderObject* newChild, RenderObject* beforeChild = 0); /* 重写了RenderObject */
    virtual void removeChild(RenderObject*);

    virtual void layoutBlock(bool relayoutChildren); /* 布局block */

    void insertPositionedObject(RenderBox*); /* 向m_positionedObjects加入position为absolute | fixed 的box */
    ...
    RootInlineBox* createAndAppendRootInlineBox(); /* 为block里面完全是inline的block创建rootinline box */

    bool generatesLineBoxesForInlineChild(RenderObject*, bool isLineEmpty = true, bool previousLineBrokeCleanly = true);

    void markAllDescendantsWithFloatsForLayout(RenderBox* floatToRemove = 0, bool inLayout = true); /* 孩子节点为float类型的情况 */
    void markPositionedObjectsForLayout(); /* 设置m_positionedObjects中的对象需要布局 */

    bool containsFloats() { return m_floatingObjects && !m_floatingObjects->isEmpty(); }
    bool containsFloat(RenderObject*);

    int lineWidth(int y, bool firstLine) const; /* inline中的content(内容)长度 */
    
    virtual int lowestPosition(bool includeOverflowInterior = true, bool includeSelf = true) const; /* block 最底点 */
    virtual int rightmostPosition(bool includeOverflowInterior = true, bool includeSelf = true) const; /* block 最右点 */
    virtual int leftmostPosition(bool includeOverflowInterior = true, bool includeSelf = true) const; /* block最左点 */
    ...
    virtual void setSelectionState(SelectionState s); /* 设置该block的选择状态,包括其containingBlock */

    GapRects selectionGapRectsForRepaint(RenderBoxModelObject* repaintContainer); /* 返回选择后要重画的三个矩形区域 */
    ...
    // Helper methods for computing line counts and heights for line counts.  如果this不含有inlinebox,则在其它里面最外层的含有inline box的block中寻找
    RootInlineBox* lineAtIndex(int); /* 返回inline中第n个linebox, */
    int lineCount();		     /* 返回inline的个数 */
...
protected:
    ...
    int maxTopPosMargin() const { return m_maxMargin ? m_maxMargin->m_topPos : MaxMargin::topPosDefault(this); }/*marginTop>0直接返回,<0返回0  */
    int maxTopNegMargin() const { return m_maxMargin ? m_maxMargin->m_topNeg : MaxMargin::topNegDefault(this); }/*marginTop<0返回绝对值,>0返回0  */
    int maxBottomPosMargin() const { return m_maxMargin ? m_maxMargin->m_bottomPos : MaxMargin::bottomPosDefault(this); }/* marginBottom>0直接返回,<0返回0 */
    int maxBottomNegMargin() const { return m_maxMargin ? m_maxMargin->m_bottomNeg : MaxMargin::bottomNegDefault(this); }/* marginBottom<0返回绝对值,>0返回0 */
    ...
    virtual bool nodeAtPoint(const HitTestRequest&, HitTestResult&, int x, int y, int tx, int ty, HitTestAction); /* 由mouseEvent调用 */
...
private:
    ...
    virtual const RenderObjectChildList* virtualChildren() const { return children(); } /* 返回可见孩子 */
    ...
    void layoutBlockChildren(bool relayoutChildren, int& maxFloatBottom); /* 布局块孩子 */
    void layoutInlineChildren(bool relayoutChildren, int& repaintTop, int& repaintBottom); /* 布局inline孩子 */
    ...
    virtual RootInlineBox* createRootInlineBox(); //(创建root inline box) Subclassed by SVG and Ruby.
    ...
    int floatBottom() const;	/* 计算当前block所含有的float object中，最低的边的高度 */
    inline int leftBottom();
    inline int rightBottom();

    int rightOffset() const;	/* borderleft + paddingleft+availableWidth */
    int leftOffset() const;	/* borderleft + paddingleft */
    ...
    struct FloatingObject : Noncopyable { /* float 对象 */
        enum Type {
            FloatLeft,
            FloatRight
        };
    ...
    int estimateVerticalPosition(RenderBox* child, const MarginInfo&); /* 估算垂直高度 */
    ...
    struct MaxMargin : Noncopyable {
        ...
        static int topPosDefault(const RenderBlock* o) { return o->marginTop() > 0 ? o->marginTop() : 0; } /*marginTop>0直接返回,<0返回0  */
        static int topNegDefault(const RenderBlock* o) { return o->marginTop() < 0 ? -o->marginTop() : 0; }/*marginTop<0返回绝对值,>0返回0  */
        static int bottomPosDefault(const RenderBlock* o) { return o->marginBottom() > 0 ? o->marginBottom() : 0; } /* marginBottom>0直接返回,<0返回0 */
        static int bottomNegDefault(const RenderBlock* o) { return o->marginBottom() < 0 ? -o->marginBottom() : 0; }/* marginBottom<0返回绝对值,>0返回0 */
