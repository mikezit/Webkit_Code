...
public:
    ...
    virtual void layout();
    virtual void calcWidth();	/* 重新计算新的width(为viewWidth) */
    virtual void calcHeight();	/* 重新计算新的height(为viewHeight) */
    ...
    virtual void computeRectForRepaint(RenderBoxModelObject* repaintContainer, IntRect&, bool fixed = false); /* 计算要重新画的矩形，IntRect是要重画的矩形区域(相对于屏幕坐标),fixed代表要画的区域的节点是固定的，position=fixed */
