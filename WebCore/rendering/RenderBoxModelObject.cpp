...
void RenderBoxModelObject::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (requiresLayer()) {	// 需要创建layer
        if (!layer()) {		// layer不存在
            if (s_wasFloating && isFloating())
                setChildNeedsLayout(true);
            m_layer = new (renderArena()) RenderLayer(this); // 创建一个新的layer
            setHasLayer(true);	 // 设置该render拥有layer
            m_layer->insertOnlyThisLayer(); // 插入该layer到父亲layer
...
void RenderBoxModelObject::paintFillLayerExtended(const PaintInfo& paintInfo, const Color& c, const FillLayer* bgLayer, int tx, int ty, int w, int h, InlineFlowBox* box, CompositeOperator op, RenderObject* backgroundObject)
{
    ...
    bool clippedWithLocalScrolling = hasOverflowClip() && bgLayer->attachment() == LocalBackgroundAttachment; // 如果有overflow的scroll bar，那么画背景的区域还要改变
    ...
    if (!bgLayer->next()) {
        ...
        if (bgColor.isValid() && bgColor.alpha() > 0)
            context->fillRect(rect, bgColor, style()->colorSpace()); // 绘制背景颜色
    }
    // 绘制图像背景
                                       ...
                                       const RenderStyle* style, bool begin, bool end)
{
    ...
    bool topTransparent = style->borderTopIsTransparent(); // 透明检测
    ...
    EBorderStyle topStyle = style->borderTopStyle(); // 样式检测
    ...
    bool renderTop = topStyle > BHIDDEN && !topTransparent; // 是否要画上边
    bool renderLeft = leftStyle > BHIDDEN && begin && !leftTransparent; // 是否要画左边
...
void RenderBoxModelObject::paintBoxShadow(GraphicsContext* context, int tx, int ty, int w, int h, const RenderStyle* s, ShadowStyle shadowStyle, bool begin, bool end)
{
    ...
    bool hasOpaqueBackground = s->backgroundColor().isValid() && s->backgroundColor().alpha() == 255; // opaque:不透明的
