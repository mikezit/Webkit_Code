...
public:
    ...
    bool hasSelfPaintingLayer() const; /* 是否自己有图层 */
    RenderLayer* layer() const { return m_layer; } /* 返回自己的图层 */
    virtual bool requiresLayer() const { return isRoot() || isPositioned() || isRelPositioned() || isTransparent() || hasOverflowClip() || hasTransform() || hasMask() || hasReflection(); } /* 是否需要一个RenderLayer */

    // This will work on inlines to return the bounding box of all of the lines' border boxes.
    virtual IntRect borderBoundingBox() const = 0;

    // Virtual since table cells override
    virtual int paddingTop(bool includeIntrinsicPadding = true) const; /* intrinsic 内在的 */
    ...
    virtual int borderTop() const { return style()->borderTopWidth(); } /* 顶边 border 宽度 */
    virtual int borderBottom() const { return style()->borderBottomWidth(); } /* 底边border宽度 */
    virtual int borderLeft() const { return style()->borderLeftWidth(); } /*左边border宽度  */
    virtual int borderRight() const { return style()->borderRightWidth(); } /* 右边border宽度 */
