...
void RenderObject::addChild(RenderObject* newChild, RenderObject* beforeChild)
{
    ...
    bool needsTable = false;	// 是否需要创建RenderTable
    ...
    if (newChild->isListItem())
        ...
        needsTable = !isTable(); // isTable()为真，说明本类已经是RenderTable不需要在得到RenderTable
    else if (newChild->isTableRow())//RenderTableRow加入到RenderTableSection中
        needsTable = !isTableSection();
    else if (newChild->isTableCell()) { // RenderTableCell加入到RenderTableRow中
    ...
    if (needsTable) {
        ...
        // Just add it...(把newchild插入到children列表中去)
...
{
    if (RenderObject* o = firstChild()) // 如果有孩子，则返回第一个孩子
        return o;

    return nextInPreOrderAfterChildren(); // 否则调用nextInPreOrderAfterChildren
}
 ...
 *	     +-----------------------------------------------------------+
 *	     |   该函数的目的：		    		                 |
 *	     |     当前节点是n0，那么找到自己的下个兄弟，如果自己没有兄弟，那么就
 *	     |  找父亲的下个兄弟，以此循环，直到找到为止.
 *	     |     在上面的例子中，如果当前节点为n0，那么此函数将返回n5
 *	     |		       		    		                 |
 *	     |                 		    		                 |
 *	     |                                                           |
 *	     +-----------------------------------------------------------+
...
{
    RenderObject* o;
    if (!(o = nextSibling())) {	// 如果没有下个兄弟了,则执行if里面的语句块,如果有下个兄弟,直接返回下个兄弟
        o = parent();		
...
RenderObject* RenderObject::nextInPreOrderAfterChildren(RenderObject* stayWithin) const
{
    ...
    RenderObject* next;
    while (!(next = current->nextSibling())) { // 如果当前RenderObject没有下个兄弟
        current = current->parent();	       // 则使当前RenderObject为父节点
        if (!current || current == stayWithin) // 如果current==stayWithin则返回0
            return 0;
    }
    return next;
}
// 如果有右兄弟，则返回友兄弟子树最右边的叶子，如果没有右兄弟则返回父亲
...
// 把obj的所在的layer加入到parentLayer的孩子中去,(newObject的作用是当beforeChild为空的情况下确定beforeChild)
static void addLayers(RenderObject* obj, RenderLayer* parentLayer, RenderObject*& newObject,
                      RenderLayer*& beforeChild)
{
    if (obj->hasLayer()) { // 如果obj有RenderLayer
        if (!beforeChild && newObject) { // 如果beforeChild不存在,则在下面计算
    ...
    // 如果obj有没有RenderLayer ,则遍历obj的每个孩子,把每个孩子的layer加入到parentLayer中去
    for (RenderObject* curr = obj->firstChild(); curr; curr = curr->nextSibling())
        addLayers(curr, parentLayer, newObject, beforeChild);
}
// RenderObjectChildList里面的调用方法为newObject->addLayer(parentLayer,newObject);
void RenderObject::addLayers(RenderLayer* parentLayer, RenderObject* newObject)
{
    if (!parentLayer)	// 如果parentLayer不存在,则不做操作返回
        return;

    RenderObject* object = newObject;
    RenderLayer* beforeChild = 0; // beforeChild 为空，表示加在childlist的最后
...
// 找到下一个layer,这个layer的父层是parentLayer
                                         ...
                                         bool checkParent)
{
    ...
    if (ourLayer && ourLayer->parent() == parentLayer) // 如果当前层的父亲就是parentLayer
        return ourLayer;			       // 则返回当前层
...
{
     RenderStyle* styleToUse = this->style();
     const Font& font = styleToUse->font();
     char text[20];
     if(s == BSTop){
	  sprintf(text,"%d,%d",x1,y1);
	  TextRun textRun(text,20);
	  graphicsContext->drawText(font,textRun,IntPoint(x1,y1));

	  sprintf(text,"%d",x2-x1);
	  TextRun textRun1(text,20);
	  graphicsContext->drawText(font,textRun1,IntPoint((x2-x1)/2,y1));
     }else if(s == BSLeft){
	  sprintf(text,"%d",y2-y1);
	  TextRun textRun(text,20);
	  graphicsContext->drawText(font,textRun,IntPoint(x1-20,(y2-y1)/2));
     }
...
// 为该render设置renderstyle
void RenderObject::setStyle(PassRefPtr<RenderStyle> style)
{
    if (m_style == style) // 如果要设置的style和原来的style在内存上都是相等的，则直接返回
        return;

    StyleDifference diff = StyleDifferenceEqual; // 前后style的不同
    unsigned contextSensitiveProperties = ContextSensitivePropertyNone;
    if (m_style)
        diff = m_style->diff(style.get(), contextSensitiveProperties); // 进行比较

    diff = adjustStyleDifference(diff, contextSensitiveProperties);

    styleWillChange(diff, style.get()); // style将改变
...
RenderObject* RenderObject::container(RenderBoxModelObject* repaintContainer, bool* repaintContainerSkipped) const
{
    ...
    // the object is not part of the primary document subtree yet.[orphan ['ɒ:fәn] a. 无双亲的]
