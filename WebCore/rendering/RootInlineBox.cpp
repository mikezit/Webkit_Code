...
void RootInlineBox::childRemoved(InlineBox* box)
{
    ...
    for (RootInlineBox* prev = prevRootBox(); prev && prev->lineBreakObj() == box->renderer(); prev = prev->prevRootBox()) {
     ...
     // heightOfBlock:传入当前inlinebox的高度
int RootInlineBox::verticallyAlignBoxes(int heightOfBlock)
{
     // const RenderLayer* enclosingLayer = renderer()->enclosingLayer();
     // const RenderLayer* rootLayer =  renderer()->enclosingLayer()->root();
     // int boxX = 0;
     // int boxY = 16+toRenderBlock(renderer())->y();
     // fprintf(stderr,"%d",boxY);
     // enclosingLayer->convertToLayerCoords(rootLayer,boxX,boxY);
     // fprintf(stderr,":%d\n",boxY);
     // int viewHeight = renderer()->view()->viewHeight(); // 网页的高度
     // fprintf(stderr,"view:%d,heightOfBlock:%d,tall:%d\n",viewHeight,heightOfBlock,(heightOfBlock+boxY));
     // if( (((heightOfBlock+boxY)%viewHeight) + lineHeight(true)) > viewHeight  )
     // 	  heightOfBlock += lineHeight(true) ;
    ...
    computeLogicalBoxHeights(maxPositionTop, maxPositionBottom, maxAscent, maxDescent, strictMode); // 计算前4个参数

    if (maxAscent + maxDescent < max(maxPositionTop, maxPositionBottom))
        adjustMaxAscentAndDescent(maxAscent, maxDescent, maxPositionTop, maxPositionBottom);

    int maxHeight = maxAscent + maxDescent; // 当前行的最大高度
    int lineTop = heightOfBlock;	    // 当前行顶端
    int lineBottom = heightOfBlock;	    // 当前行底端 
