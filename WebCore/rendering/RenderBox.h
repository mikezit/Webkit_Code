 ...
 *
 */
/* Css 中box model (整个document tree是一个RenderBox 其中 x=0,y=0,width=页宽,height=页高)*/
#ifndef RenderBox_h
#define RenderBox_h
...
public:
    ...
    int x() const { return m_frameRect.x(); } /* box 可见矩形(不包含marging)相对于 */
    int y() const { return m_frameRect.y(); } /* containing box的top left的坐标点 */
    int width() const { return m_frameRect.width(); } /* box的border的宽度和高度 */
    int height() const { return m_frameRect.height(); }
    
    void setX(int x) { m_frameRect.setX(x); } /* set box坐标 ,高宽 */
    void setY(int y) { m_frameRect.setY(y); }
    void setWidth(int width) { m_frameRect.setWidth(width); }
    void setHeight(int height) { m_frameRect.setHeight(height); }
    
    IntPoint location() const { return m_frameRect.location(); } /* 得到box的可见矩形的topleft坐标 */
    IntSize size() const { return m_frameRect.size(); } /* 得到高宽 */
    ...
    void move(int dx, int dy) { m_frameRect.move(dx, dy); } /* 移动box的位置  */

    IntRect frameRect() const { return m_frameRect; } /* 返回box 可见矩形 */
    void setFrameRect(const IntRect& rect) { m_frameRect = rect; } /* 设置可见矩形 */
    ...
    IntRect contentBoxRect() const { return IntRect(borderLeft() + paddingLeft(), borderTop() + paddingTop(), contentWidth(), contentHeight()); } /* 返回相对于containg box 的坐标的content Rect */
    // The content box in absolute coords. Ignores transforms.
    IntRect absoluteContentBox() const; /* 返回绝对坐标的content box */
    ...
    /* css中溢出管理 关于溢出 参考 RenderOverflow.h文件说明 */
    ...
    void addLayoutOverflow(const IntRect&); /* 向m_overflow增加layout over */
    void addVisualOverflow(const IntRect&); /* 向m_overflow增加visual over */
    
    void addShadowOverflow();
    void addOverflowFromChild(RenderBox* child) { addOverflowFromChild(child, IntSize(child->x(), child->y())); } /* 把孩子的溢出范围融合到m_overflow中 , 参考 RenderOverflow.h文件说明 */
    void addOverflowFromChild(RenderBox* child, const IntSize& delta); /* delta 为child的topleft点相对于本box的距离 */
    ...
    int clientWidth() const;/* More IE extensions.  clientWidth and clientHeight represent the   */
    int clientHeight() const;/*interior of an object excluding border and scrollbar. */

    // scrollWidth/scrollHeight will be the same as clientWidth/clientHeight unless the
    // object has overflow:hidden/scroll/auto specified and also has overflow.
    // scrollLeft/Top return the current scroll position.  These methods are virtual so that objects like
    // textareas can scroll shadow content (but pretend that they are the objects that are
    // scrolling).   /* 当overflow设置成scroll，有溢出时会显示scroll bar */
    virtual int scrollLeft() const; /* 水平scroll位置 */
    virtual int scrollTop() const;  /* 垂直scroll位置 */
    virtual int scrollWidth() const;
    virtual int scrollHeight() const;
    virtual void setScrollLeft(int);
    virtual void setScrollTop(int);

    virtual int marginTop() const { return m_marginTop; } /* 得到各个margin的值 */
    ...
    // methods.see http://www.w3.org/TR/CSS21/box.html#collapsing-margins for more infomation
    ...
    virtual void layout();	/* 对所有孩子进行布局 */
    virtual void paint(PaintInfo&, int tx, int ty); /* 对所有孩子进行绘制 */
    virtual bool nodeAtPoint(const HitTestRequest&, HitTestResult&, int x, int y, int tx, int ty, HitTestAction);

    virtual void destroy();

    virtual int minPrefWidth() const; /* 最小适宜宽度 */
    virtual int maxPrefWidth() const; /* 最大适宜宽度 */
    ...
    virtual IntSize offsetFromContainer(RenderObject*, const IntPoint&) const; /* 偏移计算 */
    ...
    // shifted. -dwh
    void calcHorizontalMargins(const Length& marginLeft, const Length& marginRight, int containerWidth); /* 计算m_marginLeft和计算m_marginRight */
    ...
    virtual void calcWidth();	/* 计算box的宽度,和水平margin */
    virtual void calcHeight();  /* 计算box的高度 */
    ...
    int calcWidthUsing(WidthType, int containerWidth); /* 计算box的宽度 */
    int calcHeightUsing(const Length& height);	       /* 计算box的高度 */
    ...
    bool scrollsOverflow() const { return scrollsOverflowX() || scrollsOverflowY(); } /* 是否有overflow 设置成滚动 */
    bool scrollsOverflowX() const { return hasOverflowClip() && (style()->overflowX() == OSCROLL || hasAutoHorizontalScrollbar()); } /* 有水平overflow scroll */
    bool scrollsOverflowY() const { return hasOverflowClip() && (style()->overflowY() == OSCROLL || hasAutoVerticalScrollbar()); } /* 有垂直overflow scroll */
