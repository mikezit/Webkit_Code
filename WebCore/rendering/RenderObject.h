...
/*
 *  The painting of a layer occurs in three distinct phases(不同的阶段).  Each phase involves
...
enum PaintPhase {		/* 渲染阶段 */
    PaintPhaseBlockBackground, 	/* 画自己的背景  */
    PaintPhaseChildBlockBackground, /* 画自己的背景,再画孩子们的背景 */
    PaintPhaseChildBlockBackgrounds, /* 画孩子们的背景 */
    PaintPhaseFloat,
    PaintPhaseForeground,	/* 2 */
    PaintPhaseOutline,
...
enum HitTestFilter {		/* 点击测试过滤器 */
    HitTestAll,
    HitTestSelf,
    HitTestDescendants
};

enum HitTestAction {		/* 点击测试动作 */
    HitTestBlockBackground,
    HitTestChildBlockBackground,
    HitTestChildBlockBackgrounds,
    HitTestFloat,		/* Test floats */
    HitTestForeground		/*  test the foreground layer */
};
...
public:
    ...
    RenderObject* parent() const { return m_parent; } /* 返回父亲 */
    bool isDescendantOf(const RenderObject*) const;   /* 继承关系判断 */

    RenderObject* previousSibling() const { return m_previous; } /* 上个兄弟 */
    RenderObject* nextSibling() const { return m_next; } /* 下个兄弟 */

    RenderObject* firstChild() const /* 得到第一个孩子*/
    {
        if (const RenderObjectChildList* children = virtualChildren())
            return children->firstChild();
        return 0;
    }
    RenderObject* lastChild() const /* 得到最后一个孩子 */
    {
        if (const RenderObjectChildList* children = virtualChildren())
            return children->lastChild();
        return 0;
    }
    virtual RenderObjectChildList* virtualChildren() { return 0; } /* 返回孩子列表 */
    virtual const RenderObjectChildList* virtualChildren() const { return 0; }

    RenderObject* nextInPreOrder() const; /*如果有孩子，则返回第一个孩子,否则调 ..AfterChildren*/
    RenderObject* nextInPreOrder(RenderObject* stayWithin) const; /* stayWithin 内 */
    RenderObject* nextInPreOrderAfterChildren() const; /* 见cpp图说明 */
    RenderObject* nextInPreOrderAfterChildren(RenderObject* stayWithin) const;
    RenderObject* previousInPreOrder() const; /* 见cpp */
    RenderObject* childAt(unsigned) const; /* 返回对应index的孩子 */

    RenderObject* firstLeafChild() const; /* 找到最左边的叶子 */
    RenderObject* lastLeafChild() const;  /* 找到最右边的叶子 */
    ...
    RenderLayer* enclosingLayer() const; /* 返回外层RenderLayer */
    RenderLayer* enclosingSelfPaintingLayer() const; /* 返回外层SelfPaintingLayer */
    void addLayers(RenderLayer* parentLayer, RenderObject* newObject);
    void removeLayers(RenderLayer* parentLayer); /* see cpp */
    void moveLayers(RenderLayer* oldParent, RenderLayer* newParent);/* see cpp */
    RenderLayer* findNextLayer(RenderLayer* parentLayer, RenderObject* startPoint, bool checkParent = true);

    // Convenience function for getting to the nearest enclosing box of a RenderObject.
    RenderBox* enclosingBox() const; /* 返回外层RenderBox */
    ...
    // RenderObject tree manipulation(渲染树操作)
    //////////////////////////////////////////
...
public:
    ...
    virtual bool isApplet() const { return false; } /* RecderObject 对应的类型判断 */
    ...
    virtual bool isTable() const { return false; } /* 是否是RenderTable */
    virtual bool isTableCell() const { return false; } /* 是否是RenderTableCell */
    virtual bool isTableCol() const { return false; } /* 是否是RenderTableCol */ 
    virtual bool isTableRow() const { return false; } /* 是否是RenderTableRow */
    virtual bool isTableSection() const { return false; } /* 是否是RenderTableSection */
    ...
    bool isBody() const;	/* 是否是<body>标签对应的RenderObject  */
    bool isHR() const;		/* 是否是<hr>标签对应的RenderObject */
    ...
    bool childrenInline() const { return m_childrenInline; } /* 孩子是否是inline */
    ...
    bool isAnonymous() const { return m_isAnonymous; } /* 是否是匿名节点 */
    void setIsAnonymous(bool b) { m_isAnonymous = b; } /* 设置成匿名节点 */
    ...
    bool needsLayout() const { return m_needsLayout || m_normalChildNeedsLayout || m_posChildNeedsLayout || m_needsPositionedMovementLayout; } /* 需要布局 */
    bool selfNeedsLayout() const { return m_needsLayout; } /* 自己需要布局 */
...
public:
    ...
    Node* node() const { return m_isAnonymous ? 0 : m_node; } /* 返回对应节点 */
    Document* document() const { return m_node->document(); } /* 返回document节点 */
    void setNode(Node* node) { m_node = node; } /* 设置所属节点 */
    ...
    void setPositioned(bool b = true)  { m_positioned = b;  } /* 对应于css中的position属性 absolute 和 fixed 为true */
    ...
    virtual void paint(PaintInfo&, int tx, int ty); /* 绘制对象 */

    // Recursive function that computes the size and position of this object and all its descendants.
    virtual void layout();	/* 对所有的孩子和子孙进行布局 */
    ...
    virtual void updateHitTestResult(HitTestResult&, const IntPoint&); /* 鼠标点击时得到node和点的信息 */
    ...
    // Convert the given local point to absolute coordinates /* 坐标转换 */
    // FIXME: Temporary. If useTransforms is true, take transforms into account. Eventually localToAbsolute() will always be transform-aware.
    FloatPoint localToAbsolute(FloatPoint localPoint = FloatPoint(), bool fixed = false, bool useTransforms = false) const; /* 本地坐标到绝对坐标转换 */
    FloatPoint absoluteToLocal(FloatPoint, bool fixed = false, bool useTransforms = false) const; /* 绝对坐标到本地坐标转换 */
    ...
    // When performing a global document tear-down(折毁), the renderer of the document is cleared.  We use this
    ...
    void remove() { if (parent()) parent()->removeChild(this); } /* 从父节点中移出自己 */
    ...
    virtual void mapLocalToContainer(RenderBoxModelObject* repaintContainer, bool useTransforms, bool fixed, TransformState&) const; /* 把本地坐标转换成在Container之中的坐标 */
...
private:
    ...
    Node* m_node;		/* 对应的Node节点 */

    RenderObject* m_parent; 	/* 父亲 */
    RenderObject* m_previous;	/* 上个兄弟 */
    RenderObject* m_next;	/* 下个兄弟 */
...
inline void RenderObject::markContainingBlocksForLayout(bool scheduleRelayout, RenderObject* newRoot)
{
    ...
    RenderObject* o = container(); /* 得到contaning block */
    ...
    while (o) {
        ...
        if (!container && !o->isRenderView()) /* 如果container为空并且o不是renderview */
            return;
        if (!last->isText() && (last->style()->position() == FixedPosition || last->style()->position() == AbsolutePosition)) { /* 非文字节点并且 position == fixed | absolute*/
            if ((last->style()->top().isAuto() && last->style()->bottom().isAuto()) || last->style()->top().isStatic()) { /* top,bottom=auto || top=static (自动计算?)*/
                RenderObject* parent = last->parent(); /* 得到被包含节点的父亲 */
