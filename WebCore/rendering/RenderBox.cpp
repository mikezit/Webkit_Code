...
void RenderBox::paintBoxDecorations(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (!themePainted) {
        ...
        if (!isBody() || document()->documentElement()->renderer()->style()->hasBackground())
            paintFillLayers(paintInfo, style()->backgroundColor(), style()->backgroundLayers(), tx, ty, w, h); // 绘制背景
...
bool RenderBox::pushContentsClip(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (!isControlClip && !isOverflowClip) // 如果什么clip都没有，则不必进行处理
        return false;
    
    if (paintInfo.phase == PaintPhaseOutline)
        paintInfo.phase = PaintPhaseChildOutlines;
    else if (paintInfo.phase == PaintPhaseChildBlockBackground) { // 如果是画自己和孩子的背景
        paintInfo.phase = PaintPhaseBlockBackground;		  // 先画出自己的背景
        paintObject(paintInfo, tx, ty);
        paintInfo.phase = PaintPhaseChildBlockBackgrounds; // 改成只是画孩子们的背景
    }
    IntRect clipRect(isControlClip ? controlClipRect(tx, ty) : overflowClipRect(tx, ty)); // 重新设置clipRect
    ...
    paintInfo.context->clip(clipRect);// 重新设置clipRect
    return true;
}
...
void RenderBox::popContentsClip(PaintInfo& paintInfo, PaintPhase originalPhase, int tx, int ty)
{
    ...
    if (originalPhase == PaintPhaseOutline) {
        ...
        paintInfo.phase = originalPhase; // 将phase修改成以前的状态
}
...
void RenderBox::mapLocalToContainer(RenderBoxModelObject* repaintContainer, bool fixed, bool useTransforms, TransformState& transformState) const
{
    ...
    RenderObject* o = container(repaintContainer, &containerSkipped); // 得到该RenderObject的Container Object
    if (!o)
        return;
...
{
    if (isPositioned()) {	// 此box为固定大小 position 属性为 absolute 或fixed
    ...
    if (isInline() && !isInlineBlockOrInlineTable()) { // with
    ...
    if (treatAsReplaced)
        ...
        if (width() < minW) {	// 如果前面计算出的width小于minW,则设width=minW
            setWidth(minW);
            w = style()->minWidth();
        }
    }
...
void RenderBox::calcHorizontalMargins(const Length& marginLeft, const Length& marginRight, int containerWidth)
{
    ...
    // the rule is [  'margin-left'  + 'border-left-width'  + 'padding-left'  + 'width' + 'padding-right'  + 'border-right-width'  + 'margin-right'  = width of containing block  ]
...
void RenderBox::calcHeight()
{
    ...
    if (isPositioned())
        ...
        calcVerticalMargins();	// 计算margin_top , margin_bottom
        ...
        bool checkMinMaxHeight = false; // 是否需要检查最大最小height
        ...
        if (hasOverrideSize() && parent()->isFlexibleBox() && parent()->style()->boxOrient() == VERTICAL
            ...
            h = Length(calcReplacedHeight(), Fixed); // replaced bloxd的大小为固定的
        else { // 普通情况
            h = style()->height();
...
int RenderBox::calcHeightUsing(const Length& h)
{
    ...
    if (!h.isAuto()) {
        ...
        if (height != -1) {
            height = calcBorderBoxHeight(height); // 加上border和padding的长度
            return height;
        }
    }
    return height;
...
void RenderBox::calcVerticalMargins()
{
    ...
    // the containing block (8.3) 百分比的margin是针对cotaining block的width计算的
...
{
    IntRect borderBox = borderBoxRect();
    if (borderBox.contains(rect)) // 本box的包围矩形已经包含了rect，没有包含溢出，不用添加
        return;
...
#endif        
#endif
    if (!m_overflow)		// m_overflow不存在，创建新的对象
        m_overflow.set(new RenderOverflow(borderBox));
