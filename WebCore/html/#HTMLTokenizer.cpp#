...
static const char commentStart [] = "<!--"; // 下面这些字符串用来匹配时候做比较
...
// 解析一些文本内容(主要是对其中的换行符进行处理),把list中的文字内容拷贝到m_dest中去
...
HTMLTokenizer::State HTMLTokenizer::parseNonHTMLText(SegmentedString& src, State state)
{
    ...
    while (!src.isEmpty()) {
        ...
        if (m_scriptCodeResync && !tquote && ch == '>') {
            ...
            m_scriptCodeSize = m_scriptCodeResync - 1; // 文本长度(比如<title>love</tilte>中的love的长度)
            m_scriptCodeResync = 0;		       // 重置位
            m_scriptCode[m_scriptCodeSize] = m_scriptCode[m_scriptCodeSize + 1] = 0;
            if (state.inScript())
                state = scriptHandler(state);
            else {
                state = processListing(SegmentedString(m_scriptCode, m_scriptCodeSize), state);// 解析一些文本内容(主要是对其中的换行符进行处理)
                processToken();	// 这里先处理里面的文字节点(Text)，将其加入到html树里面去
                if (state.inStyle()) { // 然后下面把当前的节点设置成闭合节点，再进行处理
        ...
        // possible end of tagname, lets check.检查完毕了，匹配
        if (!m_scriptCodeResync && !state.escaped() && !src.escaped() && (ch == '>' || ch == '/' || isASCIISpace(ch)) &&
            ...
            m_scriptCodeResync = m_scriptCodeSize-m_searchStopperLength+1; // 多余的部分，把最后的例如“</title”的长度去掉
            tquote = NoQuote;
            continue;
        }
...
HTMLTokenizer::State HTMLTokenizer::parseTag(SegmentedString& src, State state)
{
    ...
    while (!src.isEmpty()) {
        ...
        case TagName:
        {
                ...
                if (*src == commentStart[searchCount]) {// 进行注释匹配
                    ...
                    if (searchCount == 4) { // 注释匹配成功
                        ...
                        if (!src.isEmpty() && *src == '>' && m_doc->inCompatMode()) {
                          ...
                          state = parseComment(src, state); // 开始解析一个注释 
            ...
            if (m_doctypeSearchCount > 0) {
                if (toASCIILower(*src) == doctypeStart[m_doctypeSearchCount]) {
                    ...
                    m_cBuffer[cBufferPos++] = *src;
                    src.advancePastNonNewline();
                    if (m_doctypeSearchCount == 9) { // DOCTYPE匹配成功
                        // Found '<!DOCTYPE' sequence
                        state.setInDoctype(true);
                        state.setTagState(NoTag); // DOCTYPE不是一个标签
                        m_doctypeToken.reset();
                        if (inViewSourceMode())
                            m_doctypeToken.m_source.append(m_cBuffer, cBufferPos);
                        state = parseDoctype(src, state); // 开始解析一个DOCTYPE
                        m_cBufferPos = cBufferPos;
                        return state;
                    }
                    break;
	    ...
	    //不是注释也不是DocType,说明是一个标签的开始,下面扫描出标签的字符串
            bool finish = false;
            ...
            if (finish || CBUFLEN == cBufferPos) {
                bool beginTag;
                UChar* ptr = m_cBuffer; // ptr指向m_cBuffer
                ...
                m_dest = m_buffer;
                state.setTagState(SearchAttribute); //测试是否有标签属性
                cBufferPos = 0;
            }
            break;
        }
        case SearchAttribute://测试是否有标签属性
            while (!src.isEmpty()) {
                UChar curchar = *src;
                // In this mode just ignore any quotes we encounter and treat them like spaces.
                if (!isASCIISpace(curchar) && curchar != '\'' && curchar != '"') {
                    if (curchar == '<' || curchar == '>') 
                        state.setTagState(SearchEnd); // (结束属性搜索)
                    else
                        state.setTagState(AttributeName); // 开始搜索标签属性名字

                    cBufferPos = 0; // 重新置位
                    break;
                }
                if (inViewSourceMode()) // 当前字符是空格,单引号,或者双引号,说明是上个属性扫描结束,则前进字符
                    m_currentToken.addViewSourceChar(curchar);
                src.advance(m_lineNumber);
            }
            break;
        case AttributeName:// 搜索标签属性名字
        {
            ...
            while (ll--) {
                ...
                if (curchar <= '>' && (curchar >= '<' || isASCIISpace(curchar) || curchar == '/')) { // 属性名字搜索结束
                    m_cBuffer[cBufferPos] = '\0';
                    m_attrName = AtomicString(m_cBuffer); // 得到熟悉名字
                    m_dest = m_buffer;// 重置前进指针为m_buffer的开始
                    *m_dest++ = 0;    // 设置m_buffer第一个值为0 ,为搜索属性值做准备
        ...
        case SearchEqual:	// 搜索等号
            while (!src.isEmpty()) {
                ...
                if (!isASCIISpace(curchar) && curchar != '\'' && curchar != '"' && curchar != '/') { // 当前字符非空格,非单引号和双引号和slashe
                    if (curchar == '=') {
            ...
            break;
        case SearchValue:	// 搜索引号，判断出是单引号，还是双引号
            while (!src.isEmpty()) {
                UChar curchar = *src;
            ...
            break;
        case QuotedValue:	// 扫描属性的值
            while (!src.isEmpty()) {
                checkBuffer();

                UChar curchar = *src;
                if (curchar <= '>' && !src.escaped()) { // 扫描结束
                    ...
                    if ((tquote == SingleQuote && curchar == '\'') || (tquote == DoubleQuote && curchar == '\"')) { // 匹配到单引号或双引号，而且正确，得到属性的值
                        // some <input type=hidden> rely on trailing spaces. argh(除去多余的换行符)
                        while (m_dest > m_buffer + 1 && (m_dest[-1] == '\n' || m_dest[-1] == '\r'))
                            m_dest--; // remove trailing newlines
                        AtomicString attributeValue(m_buffer + 1, m_dest - m_buffer - 1);
			...
			// 遇见 <script src=http:...> 这种情况 
                        ...
                        m_currentToken.addAttribute(m_attrName, attributeValue, inViewSourceMode());
                        m_dest = m_buffer; // 重置m_dest为m_buffer的开始
                        state.setTagState(SearchAttribute); // 开始扫描下个属性
                        tquote = NoQuote;		    // tquote设置为非单引号和双引号
                        if (inViewSourceMode())
                            m_currentToken.addViewSourceChar(curchar);
                        src.advancePastNonNewline();
                        break;
        ...
        case SearchEnd:
        {
            ...
            RefPtr<Node> n = processToken(); // 开始处理标签(将得到的节点插入hmtl树)
            m_cBufferPos = cBufferPos;
            if (n || inViewSourceMode()) {
                State savedState = state;
                SegmentedString savedSrc = src;
                long savedLineno = m_lineNumber; // 开始根据标签做下一步的处理
                if ((tagName == preTag || tagName == listingTag) && !inViewSourceMode()) {
                    ...
                    if (beginTag) {
                        m_searchStopper = titleEnd; // title结尾匹配 =  "</title"
                        m_searchStopperLength = 7; // titleEnd:"</title" 长度
                        state.setInTitle(true);
                        state = parseNonHTMLText(src, state);//解析<text>标签里面的文字
                    }
...
void HTMLTokenizer::write(const SegmentedString& str, bool appendData)
{
	...
	// 需要特殊处理
            ...
            else if (state.hasTagState()) // HTML标记
                state = parseTag(m_src, state);
            else if (state.startTag()) { // 一个标记的开始(遇见<) ,进行标记判断
                state.setStartTag(false);
                
                switch (cc) {
                case '/':
                    break;
                case '!': {	// 可能是注释也可能是DOCTYPE
                    // <!-- comment --> or <!DOCTYPE ...>
                    searchCount = 1; // Look for '<!--' sequence to start comment or '<!DOCTYPE' sequence to start doctype( (在parseTag里面)向前搜索一个字符,判断是注释还是DOCTYPE)
                    m_doctypeSearchCount = 1; // 同时也要进行DOCTYPE匹配
                    break;
                }
                case '?': {
                    ...
                    tquote = NoQuote;
                    state = parseProcessingInstruction(m_src, state);
                    continue;	//执行下个while循环,根据新的state判断当前标记
		...
		// 只有在default(1)和"/"中，才会执行到这里.
                processToken();

                m_cBufferPos = 0;
                state.setTagState(TagName);
                state = parseTag(m_src, state); // 开始解析一个标签
            }
        } else if (cc == '&' && !m_src.escaped()) { // 解析实体
...
// 处理一个标签，(根据当前标签m_currentToken)返回标签对应的节点
PassRefPtr<Node> HTMLTokenizer::processToken()
{
    ...
    if (m_dest > m_buffer) {	// 说明有文字内容在当前标签里面,这步是有processListing引起的
    ...
    m_dest = m_buffer;		// 重新设置m_dest指向m_buffer开头

    RefPtr<Node> n;
    
    if (!m_parserStopped) {	// 解析没有结束
        ...
        if (inViewSourceMode())
            ...
            n = m_parser->parseToken(&m_currentToken); // 从HTMLParser中得到节点,并把节点加入到html树里面去
    }
    m_currentToken.reset();
    if (scriptController)
...
// 处理一个DoctypeToken节点,其调用HtmlParser去构造节点，然后加入到html树里面
