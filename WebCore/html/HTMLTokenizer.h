...
struct Token {
    ...
    RefPtr<NamedMappedAttrMap> attrs; /* 保存标签解析出来的属性(属性名和值对) */
    RefPtr<StringImpl> text;
    AtomicString tagName; 	/* 标签名 */
    bool beginTag;		/* 是否为开始标签 */
    bool selfClosingTag;	/* 是否为自关闭标签,例如<br/> */
    bool brokenXMLStyle;
...
private:
    ...
    enum { NoQuote, SingleQuote, DoubleQuote } tquote; /* 解析属性时用,单引号和双引号,NoQuote代表空的状态 */
    ...
    enum TagState {		/* Tag识别的一些状态(这些状态只是在parseTag函数里面使用) */
        NoTag = 0,
        TagName = 1,
        SearchAttribute = 2,
        AttributeName = 3,
