...
struct HTMLStackElem : Noncopyable {
    ...
    Node* node;			// 指向当前项所代表的节点 
    bool didRefNode;
    HTMLStackElem* next;	// 指向堆栈中的下一项
};
...
// 解析一个标签(根据Token返回一个节点)并把节点加入到html树里面去
PassRefPtr<Node> HTMLParser::parseToken(Token* t)
{
    ...
    if (!t->beginTag) {		// 这是一个结束标签,开始处理 
        processCloseTag(t);	// 涉及到当前节点出栈
        return 0;
    }
    // 这是一个开始标签
    // Ignore spaces, if we're not inside a paragraph or other inline code.
    // Do not alter the text if it is part of a scriptTag.
    if (t->tagName == textAtom && t->text && m_current->localName() != scriptTag) { // 文本内容
        ...
        while (charsLeft) {
            ...
            n = Text::createWithLengthLimit(m_document, text, charsLeft);// 创造一个Text节点Text是Node的后代 
...
void HTMLParser::parseDoctypeToken(DoctypeToken* t)
{
    ...
    if (m_document->doctype() || m_isParsingFragment || m_current != m_document)
        return;
    // 构造一个DocumentType然后将其加入到m_document中去
...
// 加入一个节点到html树里面去(flat :当当前节点为关闭节点时为true )
bool HTMLParser::insertNode(Node* n, bool flat)
{
    ...
    Node* newNode = m_current->addChild(n); // 加入到html树
    if (!newNode)
    ...
    if (tagPriority > 0 && !flat) { // 当tagPriority>0并且当前节点不是关闭节点时
        if (newNode == m_current) {
            ...
            pushBlock(localName, tagPriority); // 把当前节点入栈
...
PassRefPtr<Node> HTMLParser::getNode(Token* t)
{
    ...
    bool proceed = true;
    RefPtr<Node> result;
    if (CreateErrorCheckFunc errorCheckFunc = gFunctionMap.get(t->tagName.impl()))// 当得到上面gFunctionMap中涉及到的节点时需要进行错误检测
...
void HTMLParser::processCloseTag(Token* t)
{
    ...
    popBlock(t->tagName, checkForCloseTagErrors); // 把当前节点出栈
...
// 添加一个HTMLStackElem到栈顶,m_blockStack指向新的栈顶,level is tagPriority
void HTMLParser::pushBlock(const AtomicString& tagName, int level)
{
     int len = tagName.length();
     const UChar* name =  tagName.string().impl()->characters();
#ifdef PRINT_STACK
    printf("\nin pushBlock\n");
    printf("push tagName ");
    for(int i=0;i<len;i++){
...
void HTMLParser::printStack(){
     HTMLStackElem* elem = m_blockStack;
     printf("\nprintStatck\n");
     while (elem) {
	  AtomicString tagName = elem->tagName;
	  int level = elem->level;
	  bool strayTableContent = elem->strayTableContent;
	  bool didRefNode = elem->didRefNode;
	  int len = tagName.length();
	  const UChar* name =  tagName.string().impl()->characters();
	  printf(" tagName ");
	  for(int i=0;i<len;i++){
	  ...
	  printf(" level %d strayTableContent %d didRefNode %d\n",level,strayTableContent,didRefNode);
	  elem = elem->next;
     }
     printf("\nprintStatck over\n");
}
...
void HTMLParser::popBlock(const AtomicString& tagName, bool reportErrors)
{
     ...
     printStack();
#endif
    HTMLStackElem* elem = m_blockStack; // 让elem指向栈顶
    // 不能弹出该项
    ...
    while (elem && (elem->tagName != tagName)) { // 一般不会进入主循环，只有当进行错误检测的时候才会进入
        if (maxLevel < elem->level)
            maxLevel = elem->level;
        elem = elem->next;
    }

    if (!elem) {		// 当进行错误检测时,检测出没有错误,不会对堆栈造成影响
        if (reportErrors)
            reportError(StrayCloseTagError, &tagName, 0, true);
        return;
    }

    if (maxLevel > elem->level) { //当进行错误检测时,检测出y也许错误,修正
        // We didn't match because the tag is in a different scope, e.g.,
        // <b><p>Foo</b>.  Try to correct the problem.
        if (!isResidualStyleTag(tagName)) // 不是残余标签
            return;		// 没有问题
        return handleResidualStyleCloseTagAcrossBlocks(elem);
    }

    bool isAffectedByStyle = isAffectedByResidualStyle(elem->tagName); // 是否受style影响
    HTMLStackElem* residualStyleStack = 0; // residual:残余的，剩余的
    Node* malformedTableParent = 0;	   // malformed:难看的，畸形的
    
    elem = m_blockStack;	// elem重新指向栈顶
    unsigned stackDepth = 1;
    unsigned redundantStyleCount = 0; // redundant:多余的，失业的
    while (elem) {
