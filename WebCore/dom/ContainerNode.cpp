...
bool ContainerNode::appendChild(PassRefPtr<Node> newChild, ExceptionCode& ec, bool shouldLazyAttach)
{
    ...
    while (child) {
        ...
        if (Node* oldParent = child->parentNode()) {
            ...
            // that means someone is doing something with DOM mutation(转变,转化)- can't re-parent
        ...
        forbidEventDispatch();
        child->setParent(this);	//设置新插入的节点的父亲
        if (m_lastChild) {	// 设置前后指针
        ...
        // Add child to the rendering tree 加入节点到渲染树
        if (attached() && !child->attached() && child->parent() == this) {
            if (shouldLazyAttach)
                child->lazyAttach();
            else
...
ContainerNode* ContainerNode::addChild(PassRefPtr<Node> newChild)
{
    ...
    // It does not send any DOM mutation(变化) events.

    // Check for consistency(一致) with DTD, but only when parsing HTML.
...
{
    for (Node* child = m_firstChild; child; child = child->nextSibling()) // 先添加子节点
        child->attach();
    Node::attach();		// 再添加自己
}
