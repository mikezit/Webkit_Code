...
public:
	...
	{ return getAttributeEventListener(eventNames().attribute##Event);  }

        void setOn##attribute(PassRefPtr<EventListener> listener)
	{ setAttributeEventListener(eventNames().attribute##Event, listener); } \
     */
    ...
    String inputEncoding() const { return Document::encoding(); } /* 得到输入编码 */
    String charset() const { return Document::encoding(); } /* 得到字符编码 */
    String characterSet() const { return Document::encoding(); } /* 得到字符编码 */

    void setCharset(const String&); /* 设置字符编码 */

    String contentLanguage() const { return m_contentLanguage; }
    void setContentLanguage(const String& lang) { m_contentLanguage = lang; }

    String xmlEncoding() const { return m_xmlEncoding; } /* 得到xml编码 */
    String xmlVersion() const { return m_xmlVersion; }	 /* 得到xml编码 */
    ...
    PassRefPtr<HTMLCollection> images(); /* 得到所有的<image>标签节点  */
     ...
     * 当完成一个sheet的加载后，调用此函数更新document内部的sheet
     */
    ...
    bool usesDescendantRules() const { return m_usesDescendantRules; } /* 一些规则 */
    ...
    FrameView* view() const; // can be NULL (返回m_frame的view)
    Frame* frame() const { return m_frame; } // can be NULL (返回m_frame)
    Page* page() const; // can be NULL(返回m_frame的page)
    Settings* settings() const; // can be NULL(返回m_frame的setting)
#if ENABLE(INSPECTOR)
    ...
    virtual void attach();	/* 创建布局RenderObject,并设置该节点加入到渲染树的状态为真 */
    virtual void detach();	/* 所有节点进行detach */
    ...
    // to get visually ordered hebrew(希伯来语) and arabic(阿拉伯语) pages right
    ...
    enum ParseMode { Compat, AlmostStrict, Strict }; /* 解析模式 */

    void setParseMode(ParseMode m) { m_parseMode = m; } /* 解析摸索的一些设置和判断 */
    ...
    void setParsing(bool);  /* 设置解析状态 */
    bool parsing() const { return m_bParsing; } /* 获取解析状态 */
    int minimumLayoutDelay();

    // This method is used by Android.
    void setExtraLayoutDelay(int delay) { m_extraLayoutDelay = delay; }

    bool shouldScheduleLayout(); /* 是否应该重新调整布局 */
    int elapsedTime() const;	 /* 从本类形成开始到现在用掉的时间 */

    void setTextColor(const Color& color) { m_textColor = color; } /* 字体颜色设置 */
    Color textColor() const { return m_textColor; }

    const Color& linkColor() const { return m_linkColor; } /* 默认连接颜色 */
    const Color& visitedLinkColor() const { return m_visitedLinkColor; } /* 访问过的连接颜色 */
    const Color& activeLinkColor() const { return m_activeLinkColor; } /* 正在活跃的连接颜色 */
    void setLinkColor(const Color& c) { m_linkColor = c; } /* 设置连接颜色 */
    ...
    MouseEventWithHitTestResults prepareMouseEvent(const HitTestRequest&, const IntPoint&, const PlatformMouseEvent&); /* 处理鼠标事件(移交给rendView) */
    ...
    */
    ...
    bool setFocusedNode(PassRefPtr<Node>); /* 设置当前的焦点节点 */
    Node* focusedNode() const { return m_focusedNode.get(); }

    void getFocusableNodes(Vector<RefPtr<Node> >&); /*  得到所有可以有焦点的节点 */
    ...
    void setHoverNode(PassRefPtr<Node>); /* 设置当前鼠标位置所在的节点 */
    Node* hoverNode() const { return m_hoverNode.get(); }

    void setActiveNode(PassRefPtr<Node>); /* 设置当前活动的节点 */
    ...
    void textInserted(Node*, unsigned offset, unsigned length); /* 向节点插入文字 */
    void textRemoved(Node*, unsigned offset, unsigned length);	/* 从节点中移出文字 */
    ...
    bool hasListenerType(ListenerType listenerType) const { return (m_listenerTypes & listenerType); } /* 判断是否在监听某种事件 */
    void addListenerType(ListenerType listenerType) { m_listenerTypes = m_listenerTypes | listenerType; } /* 增加监听事件的总类 */
    void addListenerTypeIfNeeded(const AtomicString& eventType);/* 增加监听事件的总类 */
    ...
    String title() const { return m_title; }		     /* 得到页面标题 */
    void setTitle(const String&, Element* titleElement = 0); /* 设置页面标题 */
    void removeTitle(Element* titleElement);		     /* 移出当前标题 */
    ...
    HTMLElement* body() const;	/* 返回body节点 */
    void setBody(PassRefPtr<HTMLElement>, ExceptionCode&);

    HTMLHeadElement* head();	/* 返回head节点 */
    ...
    const String& iconURL() const { return m_iconURL; } /* 设置和获取地址栏上显示的icon的url */
    ...
    bool isDNSPrefetchEnabled() const { return m_isDNSPrefetchEnabled; } /* 是否支持DNS预取 */
...
private:
    ...
    String encoding() const;	/* 返回字符编码 */

    void executeScriptSoonTimerFired(Timer<Document>*);

    void updateTitle();		/* 更新标题为当前标题 */
    ...
    OwnPtr<Tokenizer> m_tokenizer; /* html解析器 */
    bool m_wellFormed;
    ...
    // elements.
    int m_pendingStylesheets;	/* 当前加载样式表的个数 */
    ...
    unsigned short m_listenerTypes; /* 监听事件 */
    ...
    Color m_linkColor;		/* 连接颜色 */
    Color m_visitedLinkColor;	/* 访问过的连接颜色 */
    Color m_activeLinkColor;	/* 当前活动的连接颜色 */
    ...
    String m_title;		/* 网页标题 1把控制字符转换成空格2去掉开始和结尾的空格3去掉多余的内部空格 */
    String m_rawTitle;		/* 原始标题(没有处理过的) */
    bool m_titleSetExplicitly;	/* 标题是否已经设置  */
    ...
    double m_startTime;		/* 开始构建这个Document时候的时间 */
    ...
    String m_contentLanguage;	/* 文本语言(好像没有用) */
    ...
    bool m_inPageCache;
    String m_iconURL;		/* 地址栏上显示的icon的url */
