...
public:
    ...
    static void startIgnoringLeaks(); /* debug时用 */
    static void stopIgnoringLeaks();

    static void dumpStatistics(); /* 输出每种节点的个数,debug时用 */
    ...
    Node* parentNode() const { return parent(); } /* 返回父节点 */
    Element* parentElement() const;
    Node* previousSibling() const { return m_previous; } /* 返回上个兄弟 */
    Node* nextSibling() const { return m_next; }/* 返回下个兄弟 */
    PassRefPtr<NodeList> childNodes();		/* 返回孩子(只有containernode才有孩子节点) */
    Node* firstChild() const { return isContainerNode() ? containerFirstChild() : 0; } /* 孩子中的第一个 */
    Node* lastChild() const { return isContainerNode() ? containerLastChild() : 0; } /* 孩子中的最后一个 */
    ...
    virtual bool insertBefore(PassRefPtr<Node> newChild, Node* refChild, ExceptionCode&, bool shouldLazyAttach = false); /* 插入一个节点(在子类中实现) */
    virtual bool replaceChild(PassRefPtr<Node> newChild, Node* oldChild, ExceptionCode&, bool shouldLazyAttach = false);/* 替换一个节点(在子类中实现) */
    virtual bool removeChild(Node* child, ExceptionCode&);/* 删除一个节点(在子类中实现) */
    virtual bool appendChild(PassRefPtr<Node> newChild, ExceptionCode&, bool shouldLazyAttach = false);/* 增加一个节点(在子类中实现) */

    void remove(ExceptionCode&);			/* 把自己从自己的父节点中删除 */
    bool hasChildNodes() const { return firstChild(); } /* 是否有孩子 */
    ...
    Node* lastDescendant() const; /* 最后一个后代(树中最左边的孩子)*/
    Node* firstDescendant() const; /* 第一个后代（树中最右边的孩子） */
    
    // Other methods (not part of DOM)

    bool isElementNode() const { return m_isElement; }
    bool isContainerNode() const { return m_isContainer; } /* 是否为容器节点(包含子节点) */
    bool isTextNode() const { return m_isText; }

    /* 对于下面的这些虚函数，在对应的子函数有实现，把对应的返回ture就可以了 */
    ...
    // The node's parent for the purpose of event capture and bubbling(冒泡).
    ...
    // These low-level calls give the caller responsibility for maintaining the integrity of the tree.(设置左右兄弟姐妹)
    ...
    virtual void setHovered(bool b = true) { m_hovered = b; } /* 鼠标在上 */
    ...
    virtual bool supportsFocus() const; /* 是否支持焦点 */
    // Whether the node can actually be focused.
    virtual bool isFocusable() const;	/* 是否可设置焦点 */
    virtual bool isKeyboardFocusable(KeyboardEvent*) const; /* 键盘是否可以设置焦点 */
    virtual bool isMouseFocusable() const; /* 是否鼠标可设置焦点 */

    virtual bool isContentEditable() const; /* 是否该节点内容可编辑 */
    virtual bool isContentRichlyEditable() const;
    virtual bool shouldUseInputMethod() const; /* 是否可以调用输入法 */
    virtual IntRect getRect() const;	       /* 返回包围该节点的矩形 */

    virtual void recalcStyle(StyleChange = NoChange) { } /* 重新计算style */

    unsigned nodeIndex() const;	/* 当前节点在父节点中的子节点中位置 */
    ...
    bool isReadOnlyNode() const { return nodeType() == ENTITY_REFERENCE_NODE; } /* 是否为只读节点 */
    virtual bool childTypeAllowed(NodeType) { return false; }
    unsigned childNodeCount() const { return isContainerNode() ? containerChildNodeCount() : 0; } /* 子孙节点的个数 */
    Node* childNode(unsigned index) const { return isContainerNode() ? containerChildNode(index) : 0; } /* 返回第index个子节点 */
    ...
    RenderObject* renderer() const { return m_renderer; } /* 返回当前的render */
    RenderObject* nextRenderer(); /* 返回下个兄弟的render */
    RenderObject* previousRenderer(); /* 返回上个兄弟的render */
    void setRenderer(RenderObject* renderer) { m_renderer = renderer; } /* 设置当前节点的渲染模块 */
    ...
    bool isDescendantOf(const Node*) const; /* 本节点是否为参数节点的后代 */
    bool contains(const Node*) const;	    /* 本节点是否包含参数节点,(是后代或相同) */
     ...
     */
    virtual void attach(); /* 设置该节点加入到渲染树的状态为真*/
    ...
    virtual void willRemove();	/* 没有实现 */
    void createRendererIfNeeded(); /* 为当前节点设置render并将其加入到render树中 */
...
private:
    ...
    unsigned containerChildNodeCount() const; /* 子孙节点的个数 */
    ...
    Document* m_document;	/* 根? */
    Node* m_previous; 		/* 上个兄弟节点 *//* 这些组成一个父节点的孩子链表 */
    Node* m_next;		/* 下个兄弟节点 */
    RenderObject* m_renderer;	/* 渲染模块 */

    unsigned m_styleChange : 2;
    bool m_hasId : 1;
    bool m_hasClass : 1;
    bool m_attached : 1;	/* 该节点是否加入到渲染树中 */
