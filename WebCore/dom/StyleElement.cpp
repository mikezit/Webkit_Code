...
void StyleElement::process(Element* e)
{
    ...
    for (Node* c = e->firstChild(); c; c = c->nextSibling()) { // 遍历样式表节点的子节点，这些节点是样式内容
    ...
    UChar* text;
    String sheetText = String::createUninitialized(resultLength, text); // 初始化样式内容字符串

    UChar* p = text;		// 从子节点得到样式内容字符窜
...
void StyleElement::createSheet(Element* e, const String& text)
{
    ...
    if (type.isEmpty() || (e->isHTMLElement() ? equalIgnoringCase(type, "text/css") : (type == "text/css"))) {
        ...
        if (screenEval.eval(mediaList.get()) || printEval.eval(mediaList.get())) {
            ...
            setLoading(true);	// 加载层叠样式表开始
            m_sheet = CSSStyleSheet::create(e, String(), KURL(), document->inputEncoding()); // 创建层叠样式表
            m_sheet->parseString(text, !document->inCompatMode()); // 开始解析
            m_sheet->setMedia(mediaList.get());
            m_sheet->setTitle(e->title());
            setLoading(false);  // 加载层叠样式表结束
        }
    }
