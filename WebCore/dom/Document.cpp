 ...
 *  1. Convert control characters to spaces(把控制字符转换成空格)
 *  2. Trim leading and trailing spaces(去掉开始和结尾的空格)
 *  3. Collapse internal whitespace.(去掉多余的内部空格)
 */
...
{   
     if (!titleElement) {	// titleElement为空
    ...
    } else if (titleElement != m_titleElement) { // 标题和上次的不一样，改动了
...
void Document::attach()
{
    ...
    RenderObject* render = renderer(); // 备份 render
    setRenderer(0);

    ContainerNode::attach();	// 这一步只是让Node中的attached 设置为 ture

    setRenderer(render); 	// 恢复render
}
...
void Document::implicitOpen()
{
    ...
    removeChildren();		// 清除所有的孩子

    m_tokenizer = createTokenizer(); // 创建新的解析器
    setParsing(true);		     // 设置状态为正在解析
