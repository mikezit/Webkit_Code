...
// 如果该node能创建一个render，则为该node创建renderobject
    ...
    if (parentRenderer && parentRenderer->canHaveChildren()
        ...
        if (rendererIsNeeded(style.get())) {
            if (RenderObject* r = createRenderer(document()->renderArena(), style.get())) {
                if (!parentRenderer->isChildAllowed(r, style.get())) // 父亲节点不能有这个render子节点
                    r->destroy(); // 销毁
                else {
                    setRenderer(r); // 设置该节点的render
                    renderer()->setAnimatableStyle(style.release()); // 为该render设置一些东西
                    parentRenderer->addChild(renderer(), nextRenderer()); // 加入到父亲render
                }
            }
        }
    }
...
void Node::showTreeAndMark(const Node* markedNode1, const char* markedLabel1, const Node* markedNode2, const char * markedLabel2) const
{
    ...
    while (node->parentNode() && !node->hasTagName(bodyTag)) // 遍历到body节点
        node = node->parentNode();
    rootNode = node;		// rootNode是body节点 
