...
double CachedResource::currentAge() const
{
    ...
    double dateValue = m_response.date(); // 网络回答的时间
...
{
    if (size == m_decodedSize)	// 如果之前的大小和将要设置的大小一样，则返回
        return;

    int delta = size - m_decodedSize; // 变化的量

    // The object must now be moved to a different queue, since its size has been changed.
    // We have to remove explicitly before updating m_decodedSize, so that we find the correct previous( 因为queue的位置是由资源的size映射的)
    // queue.
    if (inCache())	// 如果资源在cache中,则先将资源移出LRU列表
        cache()->removeFromLRUList(this);
    
    m_decodedSize = size;
   
    if (inCache()) { 	// 如果资源之前在cache中,则再将资源加入到LRU列表
