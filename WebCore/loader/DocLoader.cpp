...
{
    if (m_allowStaleResources) //stale [steil] adj.不新鲜的,陈腐的,失效的 
        return; // Don't reload resources while pasting

    if (fullURL.isEmpty())	// 检查是否为空地址
        return;
    
    if (m_reloadedURLs.contains(fullURL.string())) // 检查是否已经加载过
        return;
    
    CachedResource* existing = cache()->resourceForURL(fullURL.string());// 检查是否已经加载过
...
{
    if (Frame* f = frame()) { 	// 查看FrameLoaderClient是否允许加载图片
...
{
    KURL fullURL = m_doc->completeURL(url); // 得到网址

    if (!fullURL.isValid() || !canRequest(type, fullURL)) // 网络地址错误,不能请求
        return 0;
    ...
    CachedResource* resource = cache()->requestResource(this, type, fullURL, charset, isPreload); // 在cache中请求资源
    if (resource) {
...
{
    bool hasRendering = m_doc->body() && m_doc->body()->renderer(); // 有body,并且body有renderer
    if (!hasRendering && (referencedFromBody || type == CachedResource::ImageResource)) {
        ...
        m_pendingPreloads.append(pendingPreload); // 这样的话,把该资源加入到m_pendingPreloads队列，以后再加载
        return;
    }
    requestPreload(type, url, charset); // 开始加载资源
}
