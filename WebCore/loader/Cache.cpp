...
CachedResource* Cache::requestResource(DocLoader* docLoader, CachedResource::Type type, const KURL& url, const String& charset, bool requestIsPreload)
{
    ...
    CachedResource* resource = resourceForURL(url.string()); // 在m_resources表中查找　
    
    if (resource && requestIsPreload && !resource->isPreloaded())
        return 0;
    
    if (SecurityOrigin::restrictAccessToLocal() && !SecurityOrigin::canLoad(url, String(), docLoader->doc())) { // 安全检测?
    ...
    if (!resource) {  // 如果资源不存在Cache中,则重新load,load后加入cache
        // The resource does not exist. Create it.
        resource = createResource(type, url, charset); // 创建一个新的CachedResource对象
        ASSERT(resource);

        // Pretend(假装,装作) the resource is in the cache, to prevent it from being deleted during the load() call.
        // FIXME: CachedResource should just use normal refcounting instead.
        resource->setInCache(true);
        
        resource->load(docLoader); // 最终最进入到Class Loader里面
        
        if (resource->errorOccurred()) { // 加载出现错误
    ...
    if (resource->type() != type) // 得到的资源类型和要求的不符合
        return 0;
...
void Cache::pruneLiveResources()
{
    ...
    while (current) {
        ...
        if (current->isLoaded() && current->decodedSize()) {
            ...
            double elapsedTime = currentTime - current->m_lastDecodedAccessTime; // 该资源距离上次使用的时间间隔
            if (elapsedTime < cMinDelayBeforeLiveDecodedPrune) // 该时间太短，说明该资源和在列表中该资源之前的资源,可能以后还会使用,返回.
                return;
...
void Cache::pruneDeadResources()
{
    ...
    // dead资源超过了它的容量,进行裁减
    unsigned targetSize = static_cast<unsigned>(capacity * cTargetPrunePercentage); // Cut by a percentage to avoid immediately pruning again.
    int size = m_allResources.size(); // LRU列表个数
    
    if (!m_inPruneDeadResources) {
        // See if we have any purged resources we can evict.
        for (int i = 0; i < size; i++) {
            CachedResource* current = m_allResources[i].m_tail; // 当前LRU列表末尾
            while (current) {
                CachedResource* prev = current->m_prevInAllResourcesList; // 得到上一个CachedResource
...
void Cache::evict(CachedResource* resource)
{
    ...
    if (resource->inCache()) {
        ...
        if (delta)
            adjustSize(resource->hasClients(), delta); // 重新调整m_liveSize或则m_deadSize,当resource->hasClients()为ture时，说明调整m_liveSize大小
    } else
...
// 能够计算 大于2的0次方 到 小于 2的32次方的 i ,返回结果为 0 - 32
static inline unsigned fastLog2(unsigned i)
{
    unsigned log2 = 0;
    if (i & (i - 1))	// 检测i是否是以2为底的指数
        log2 += 1;	// 不是,则加1,取近似值
    if (i >> 16)		//检测i是否大于2的16次方
        log2 += 16, i >>= 16;	// 如果大于,则,log2加上16,i左移16位
    if (i >> 8)
        log2 += 8, i >>= 8;
    if (i >> 4)
        log2 += 4, i >>= 4;   
...
// Calls to put the cached resource out of LRU lists.
void Cache::removeFromLRUList(CachedResource* resource)
{
    ...
    CachedResource* next = resource->m_nextInAllResourcesList; //所属LRU链表中上个资源
    CachedResource* prev = resource->m_prevInAllResourcesList; //所属LRU链表中下个资源 
    
    if (next == 0 && prev == 0 && list->m_head != resource) // 链表中不存在这个资源
        return;
    
    resource->m_nextInAllResourcesList = 0; // 重置
...
// put the cached resource into LRU lists's head
void Cache::insertInLRUList(CachedResource* resource)
{
    ...
    LRUList* list = lruListFor(resource); // 得到链表
