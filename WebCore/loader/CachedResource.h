...
public:
    ...
    bool hasClients() const { return !m_clients.isEmpty(); } /* 如果这个资源有Clients，说明这个资源是live */
    ...
    unsigned count() const { return m_clients.size(); } /* CachedResourceClient个数  */

    Status status() const { return m_status; } /* 得到资源状态 */

    unsigned size() const { return encodedSize() + decodedSize() + overheadSize(); }
    unsigned encodedSize() const { return m_encodedSize; } /*encodedSize应该是资源内存的大小  */
    unsigned decodedSize() const { return m_decodedSize; } /*decodedSize代表什么?  */
    unsigned overheadSize() const;
    
    bool isLoaded() const { return !m_loading; }
    void setLoading(bool b) { m_loading = b; }

    virtual bool isImage() const { return false; } /* 该资源是否是图像 */

    unsigned accessCount() const { return m_accessCount; } /* 该资源存取次数 */
...
protected:
    ...
    ResourceResponse m_response; /* 资源的网络回答(在网络模块) */
...
private:
    ...
    double currentAge() const;	/* 该资源的年龄 */
    double freshnessLifetime() const;

    unsigned m_encodedSize;	/* 这两个size和该资源在32个LRU表中的位置有关 */
...
protected:
    bool m_inCache;		/* 资源是否在Cache中 */
    bool m_loading;		/* 资源是否正在加载 */
#ifndef NDEBUG
    bool m_deleted;
    unsigned m_lruIndex;	/* 资源在CACHE 中的 LRUList的位置 */
#endif

private:
    CachedResource* m_nextInAllResourcesList; /* CACHE 中的 LRUList 链表结构 */
    CachedResource* m_prevInAllResourcesList;
    
    CachedResource* m_nextInLiveResourcesList; /* LiveDecodedResourcesList 链表结构 */
