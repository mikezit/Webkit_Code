...
public:
    ...
    CachedImage* requestImage(const String& url); /* 请求资源 */
    ...
    CachedResource* cachedResource(const String& url) const { return m_documentResources.get(url).get(); }    /* 得到缓存的资源 */
    
    typedef HashMap<String, CachedResourceHandle<CachedResource> > DocumentResourceMap;
    const DocumentResourceMap& allCachedResources() const { return m_documentResources; } /* 得到所有缓存的资源 */

    bool autoLoadImages() const { return m_autoLoadImages; } /* 自动加载图片? */
    ...
    void removeCachedResource(CachedResource*) const; /* 清除传入的缓存资源 */
    ...
    void clearPreloads();
    void clearPendingPreloads(); /* 清除m_pendingPreloads列表中的资源请求 */
    void preload(CachedResource::Type, const String& url, const String& charset, bool referencedFromBody); /*请求资源 */
    void checkForPendingPreloads();	/* 处理m_pendingPreloads列表中的资源请求 */
    void printPreloadStats();
    
private:
    CachedResource* requestResource(CachedResource::Type, const String& url, const String& charset, bool isPreload = false); /* 请求资源 */
    void requestPreload(CachedResource::Type, const String& url, const String& charset);// 执行请求
    ...
    Cache* m_cache;		    /* 缓存 */
    HashSet<String> m_reloadedURLs; /* 加载过的资源地址 */
    mutable DocumentResourceMap m_documentResources;
    Document* m_doc;
    
    int m_requestCount;		/* 发送给Loader的Request的个数 */
    
    ListHashSet<CachedResource*> m_preloads; /* 预请求列表 */
    struct PendingPreload {
        CachedResource::Type m_type;
        String m_url;
        String m_charset;
    };
    Vector<PendingPreload> m_pendingPreloads;/* 预请求阻塞列表 */
