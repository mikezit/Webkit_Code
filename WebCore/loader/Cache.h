...
public:
    ...
    struct LRUList {		/* LRU是Least Recently Used的缩写,最近最少使用 */
        CachedResource* m_head;	/* 头指针 */
        CachedResource* m_tail;	/* 尾指针 */
        LRUList() : m_head(0), m_tail(0) { }
    };

    struct TypeStatistic {	/* 统计 */
        int count;
        int size;
        int liveSize;
        int decodedSize;
    ...
    //  - minDeadBytes: The minimum number of bytes that dead resources should consume when the cache is under pressure.
    ...
    void setPruneEnabled(bool enabled) { m_pruneEnabled = enabled; } /* prune [pru:n] vt.修剪,砍掉,削减 vi.删除 */
    void prune()
    {
    ...
    CachedResource* resourceForURL(const String&); /* 从cache中得到资源 */
...
private:
    ...
    LRUList* lruListFor(CachedResource*); /* 得到CachedResource对应的队列 */
    ...
    void evict(CachedResource*); /* evict [英] [ɪˈvɪkt] vt.(依法从房屋里或土地上)驱逐, 赶出 */
