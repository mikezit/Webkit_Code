...
Loader::Priority Loader::determinePriority(const CachedResource* resource) const
{
    ...
    switch (resource->type()) {
        ...
        case CachedResource::FontResource:
            return Medium;
        case CachedResource::ImageResource: // 图片的优先级别最低
            return Low;
    }
    ASSERT_NOT_REACHED();
    return Low;
...
{
    ASSERT(docLoader);
    Request* request = new Request(docLoader, resource, incremental, securityCheck, sendResourceLoadCallbacks); // 创建一个资源请求
    ...
    bool hadRequests = host->hasRequests(); // 在此之前有没有请求
    Priority priority = determinePriority(resource); // 得到该资源的优先级别
    host->addRequest(request, priority); // 把该资源的请求加入到主机服务器的对应级别的待处理列表中去
    docLoader->incrementRequestCount();	 // 增加对应docloader中的资源请求数
...
void Loader::nonCacheRequestInFlight(const KURL& url)
{
    ...
    AtomicString hostName = url.host(); // 得到主域名
    m_hosts.checkConsistency();
    RefPtr<Host> host = m_hosts.get(hostName.impl());
    if (!host) {		// 首次使用该域名,创建服务器
        host = Host::create(hostName, maxRequestsInFlightPerHost);
        m_hosts.add(hostName.impl(), host);
    }
...
{
    if (!m_requestsLoading.isEmpty()) // 正在处理的请求表中是否有请求
        return true;
    for (unsigned p = 0; p <= High; p++) { // 待请求的资源队列列表是否有请求
...
void Loader::Host::servePendingRequests(Loader::Priority minimumPriority)
{
    ...
    bool serveMore = true;
    for (int priority = High; priority >= minimumPriority && serveMore; --priority) // 如果有较高的优先级别没有处理，则先处理高优先级别的等待
...
void Loader::Host::servePendingRequests(RequestQueue& requestsPending, bool& serveLowerPriority)
{
    while (!requestsPending.isEmpty()) { 
        Request* request = requestsPending.first(); // 得到该优先级别队列的第一个请求
        ...
        requestsPending.removeFirst(); // 移出第一个请求(第二个将变为第一个)
        ...
        if (loader) {
            m_requestsLoading.add(loader.release(), request); // 加入到正在加载的列表
